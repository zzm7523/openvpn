 ...
 * table implementation using Bob Jenkins' hash function.
 *
...
bool hash_remove_fast (struct hash *hash, struct hash_bucket *bucket,
		const void *key, uint32_t hv);
...
static inline bool
hash_check (struct hash *hash)
{
	struct hash_bucket *bucket;
	struct hash_element *prev, *next;
	int size = 0, i;
	...
	for (i = 0; i < hash->n_buckets; ++i)
	{
		...
		while (next)
		{
			...
			{
				if (next->hash_value != prev->hash_value)
					return false;
				if ((*hash->compare_function)(next->key, prev->key))
					return false;
			}
...
static inline void
hash_add_fast (struct hash *hash, struct hash_bucket *bucket,
