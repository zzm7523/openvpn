...
/* Maximum number of parameters associated with an option, including the option name itself. */
#define MAX_PARMS  16

/* Max size of options line and parameter. */
#define OPTION_PARM_SIZE  512
#define OPTION_LINE_SIZE  2048
...
#define CIPHER_LIST_SIZE	64
#define AUTH_LIST_SIZE		8

#ifdef ENABLE_CRYPTO
	/* Cipher parms */
	const char *shared_secret_file;
	const char *shared_secret_file_inline;
	const char *tls_auth_authname;
	bool tls_auth_authname_defined;
	int key_direction;
	bool ciphername_defined;
	int n_ciphername;
	const char *ciphername[CIPHER_LIST_SIZE + 1];	/* 必须加1, 空终止数组 */
	bool authname_defined;
	int n_authname;
	const char *authname[AUTH_LIST_SIZE + 1];	/* 必须加1, 空终止数组 */
	int keysize;
	...
	/* Data channel key handshake must finalize within n seconds of handshake initiation. */
	...
	bool integration;
	bool integration_completed;
#define MAX_POLICY_SIZE    512
	struct buffer_list *policys;
...
#define MIN_DEVICE_CHECK_TIMEOUT	60
#define MAX_DEVICE_CHECK_TIMEOUT	600
	int device_check_timeout;

	const char *encrypt_cert_file;
	const char *encrypt_priv_key_file;
	const char *encrypt_pkcs12_file;

	const char *encrypt_cert_file_inline;
	char *encrypt_priv_key_file_inline;
	const char *encrypt_pkcs12_file_inline; /* contains the base64 encoding of pkcs12 file */
#endif

#ifdef TARGET_LINUX
	bool bind_cpu;
#endif
	/* 工作线程数(default = 0, 根据加密设备数自动决定) */
	int worker_thread;

#ifdef ENABLE_TUN_THREAD
#define MIN_PACKET_QUEUE_LEN	2 * RECLAIM_THRESHOLD  /* 必须大于等于 2 * RECLAIM_THRESHOLD */
#else
#define MIN_PACKET_QUEUE_LEN	4 * RECLAIM_THRESHOLD  /* 必须大于等于 4 * RECLAIM_THRESHOLD */
#endif
#define MAX_PACKET_QUEUE_LEN	65536 /* 必须小于 MAX_PACKET_BUFFER_LIST / 2 */
	int packet_queue_len;	/* 全局自由缓存列表最大值(default = 1024) */
...
#define OPT_P_PROPRIETARY     (1<<29)

#define OPT_P_DEFAULT   (~(OPT_P_INSTANCE|OPT_P_PULL_MODE|OPT_P_PEER_ID|OPT_P_PROPRIETARY))
...
bool apply_push_options (struct options *options, struct buffer *buf, unsigned int permission_mask,
...
void options_server_import (struct options *o, const char *filename, int msglevel,
		unsigned int permission_mask, unsigned int *option_types_found, struct env_set *es);
