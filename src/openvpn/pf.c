	...
	if (strcmp (line, "unknown"))
	{
		...
		if (div)
		{
			*div++ = '\0';
			...
			{
				msg (D_PF_INFO, "PF: %s/%d: bad '/n' subnet specifier: must be between 0 and 32: '%s'",
					prefix, line_num, div);
				return false;
			}
		}
		...
		if ((network.s_addr & htonl (netmask)) != network.s_addr)
		{
			...
			msg (M_WARN, "WARNING: PF: %s/%d: incorrect subnet %s/%d changed to %s/%d",
...
{
	return hash_func ((uint8_t *) key, (uint32_t) strlen ((char *) key) + 1, iv);
}
...
static void
pf_cn_test_print (const char *prefix, const int type, const char *prefix2,
		const char *cn, const bool allow, const struct pf_cn *rule)
{
	if (rule)
	{
...
static void
pf_addr_test_print (const char *prefix, const char *prefix2, const struct context *src,
		const struct mroute_addr *dest, const bool allow, const struct ipv4_subnet *rule)
{
...
lookup_cn_rule (struct hash *h, const char *cn, const uint32_t cn_hash)
{
	...
	return he ? (struct pf_cn *) he->value : NULL;
}
...
pf_check_reload (struct context *c)
{
	...
	if (c->c2.pf.enabled && c->c2.pf.filename
		&& event_timeout_trigger (&c->c2.pf.reload, &c->c2.timeval, ETT_DEFAULT))
	{
		...
		if (!platform_stat (c->c2.pf.filename, &s))
		{
			if (s.st_mtime > c->c2.pf.file_last_mod)
			{
				...
				MUTEX_LOCK (&g_pf_mutex, MAIN_THREAD_INDEX, S_PF);	/* 阻止tun线程访问pf规则 */
#endif

				pfs = pf_init_from_file (c->c2.pf.filename);
				if (pfs)
				{
					if (c->c2.pf.pfs)
...
#ifdef ENABLE_TUN_THREAD
				MUTEX_UNLOCK (&g_pf_mutex, MAIN_THREAD_INDEX, S_PF);
#endif
			}
		}

		{
			...
			if (!c->c2.pf.pfs && c->c2.pf.n_check_reload < wakeup_transition)
				wakeup = fast_wakeup;
			event_timeout_init (&c->c2.pf.reload, wakeup, now_sec (MAIN_THREAD_INDEX));
...
pf_load_from_buffer_list (struct context *c, const struct buffer_list *config)
{
	...
	pfs = pf_init (config, "[SERVER-PF]", false);
	if (pfs)
	{
		if (c->c2.pf.pfs)
			pf_destroy (c->c2.pf.pfs);
...
pf_init_context (struct context *c)
{
	...
	if (plugin_defined (c->plugins, OPENVPN_PLUGIN_ENABLE_PF))
	{
		...
		if (pf_file)
		{
			...
			{
				event_timeout_init (&c->c2.pf.reload, 1, now_sec (MAIN_THREAD_INDEX));
	...
	if (s)
	{
		...
		if (s->hash_table)
		{
			...
			while ((he = hash_iterator_next (&hi)))
			{
				...
				msg (lev, "   %s %s", e->cn, drop_accept (!e->exclude));
			}
...
{
	/* tun线程不会修改pf规则, 主线程读访问不需锁定g_pf_mutex */
