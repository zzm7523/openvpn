...
#define FRAG_SIZE_ROUND_MASK ((1 << FRAG_SIZE_ROUND_SHIFT) - 1)	/**< Bit mask for %fragment size rounding. */
 ...
 *     is also set to zero.
 */
void fragment_incoming (struct context *c, struct fragment_master *f, struct packet_buffer *buf,
 ...
 */
void fragment_outgoing (struct context *c, int thread_idx, struct fragment_master *f, struct packet_buffer *buf,
 ...
 *     sent.
 */
bool fragment_ready_to_send (struct fragment_master *f, struct packet_buffer *buf, const struct frame *frame);
...
fragment_prepend_flags (struct buffer *buf, int type, int seq_id, int frag_id, int frag_size)
{
	...
	{
		/* If you want to set FRAG_EXTRA_MASK/FRAG_EXTRA_SHIFT bits, do it here. */
		dmsg (D_FRAG_DEBUG,
