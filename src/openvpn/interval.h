...
#endif
		top->future_trigger = now_sec (MAIN_THREAD_INDEX) + wakeup;
	}
}

/*
 * Once an action is triggered, interval_test will remain true for horizon seconds.
 */
static inline void
interval_action (struct interval *top)
{
	...
	top->last_action = now_sec (MAIN_THREAD_INDEX);
}
...
static inline void
event_timeout_reset (struct event_timeout *et, time_t now_sec)
{
	if (et->defined)
	{
#ifdef PERF_STATS_CHECK
		ASSERT (now_sec >= (et->last - 1));
		et->last_last = et->last;
#endif
		et->last = now_sec;
	}
}
...
{
	CLEAR (*obj);
	ASSERT (!openvpn_gettimeofday (&obj->start, NULL, MAIN_THREAD_INDEX));
}

static inline void
usec_timer_end (struct usec_timer *obj)
{
	ASSERT (!openvpn_gettimeofday (&obj->end, NULL, MAIN_THREAD_INDEX));
}
*/
