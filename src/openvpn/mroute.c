...
static unsigned int
mroute_extract_addr_arp (struct mroute_addr *src, struct mroute_addr *dest, const struct buffer *buf)
{
	unsigned int ret = 0;
...
unsigned int
mroute_extract_addr_ipv4 (struct mroute_addr *src, struct mroute_addr *dest, const struct buffer *buf)
{
	unsigned int ret = 0;
	if (BLEN (buf) >= 1)
	{
		...
		case 6:
			...
			break;
		default:
			msg (M_WARN, "IP packet with unknown IP version=%d seen", OPENVPN_IPH_GET_VER (*BPTR (buf)));
		}
	}
	return ret;
}
...
unsigned int
mroute_extract_addr_ether (struct mroute_addr *src, struct mroute_addr *dest, struct mroute_addr *esrc,
		struct mroute_addr *edest, const struct buffer *buf)
{
	unsigned int ret = 0;
...
/*
 * Translate a struct openvpn_sockaddr (osaddr) to a struct mroute_addr (addr).
 */
...
mroute_addr_print_ex (const struct mroute_addr *ma, const unsigned int flags, struct gc_arena *gc)
{
	...
	if (ma)
	{
		...
		case MR_ADDR_IPV6:
			{
				buf_printf (&out, "%s", print_in6_addr (*(struct in6_addr*) &maddr.addr, 0, gc)); 
				if (maddr.type & MR_WITH_NETBITS)
				{
					buf_printf (&out, "/%d", maddr.netbits);
				}
...
/*
 * mroute_helper's main job is keeping track of currently used CIDR netlengths,
 * so we don't have to cycle through all 33.
 */
 ...
 */
void
mroute_helper_add_iroute6 (struct mroute_helper *mh, const struct iroute_ipv6 *ir6)
{
#ifdef THREAD_ACCESS_CHECK
	ASSERT (is_main_thread ());
#endif
...
void
mroute_helper_del_iroute6 (struct mroute_helper *mh, const struct iroute_ipv6 *ir6)
{
#ifdef THREAD_ACCESS_CHECK
	ASSERT (is_main_thread ());
#endif
