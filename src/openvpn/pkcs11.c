...
__mytime (void)
{
	return openvpn_time (NULL, THREAD_SELF_INDEX ());
}
		...
		const pkcs11h_token_id_t token, const unsigned retry)
{
	...
	openvpn_snprintf (token_resp.username, sizeof (token_resp.username),
		"Please insert %s token", token->label);
...
static PKCS11H_BOOL
_pkcs11_openvpn_pin_prompt (void * const global_data, void * const user_data, const pkcs11h_token_id_t token,
...
cleanup:
	dmsg (D_PKCS11_DEBUG, "PKCS#11: pkcs11_initialize - return %ld-'%s'", rv, pkcs11h_getMessage (rv));
	return rv == CKR_OK;
}
		...
		const unsigned private_mode, const bool cert_private)
{
	...
	dmsg (D_PKCS11_DEBUG, "PKCS#11: pkcs11_addProvider - return rv=%ld-'%s'", rv, pkcs11h_getMessage (rv));
...
pkcs11_management_id_get (const int index, char **id, char **base64)
{
	...
	*base64 = NULL;
	...
	if (entry == NULL)
	{
		dmsg (D_PKCS11_DEBUG, "PKCS#11: pkcs11_management_id_get - no certificate at index=%d", index);
		goto cleanup;
	}
	...
	if ((rv = pkcs11h_certificate_create (
			entry->certificate_id,
			NULL,
			PKCS11H_PROMPT_MASK_ALLOW_ALL,
	...
	if ((rv = pkcs11h_certificate_getCertificateBlob (
			certificate,
			NULL,
...
tls_ctx_use_pkcs11 (struct tls_root_ctx * const ssl_ctx, bool pkcs11_id_management, const char * const pkcs11_id)
{
	...
	if (pkcs11_id_management)
	{
		...
		openvpn_snprintf (id_resp.username, sizeof (id_resp.username), "Please specify PKCS#11 id to use");
		...
		if ((rv = pkcs11h_certificate_deserializeCertificateId (
				&certificate_id,
				id_resp.password
			)) != CKR_OK)
		{
...
static PKCS11H_BOOL
_pkcs11_openvpn_show_pkcs11_ids_pin_prompt (void * const global_data, void * const user_data, const pkcs11h_token_id_t token,
		const unsigned retry, char * const pin, const size_t pin_max) 
{
	...
	if (!get_console_input (BSTR (&pass_prompt), false, pin, (int) pin_max))
	{
