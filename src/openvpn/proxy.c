...
static bool
recv_line (socket_descriptor_t sd, char *buf, int len, const int timeout_sec,
		const bool verbose, struct buffer *lookahead, volatile int *signal_received)
{
	struct buffer la;
	int lastc = 0;
	...
	while (true)
	{
		...
		/* read single char */
		size = recv (sd, (char *) &c, 1, MSG_NOSIGNAL);
...
send_line_crlf (socket_descriptor_t sd, const char *src)
{
	...
	ASSERT (buf_write (&buf, src, (int) strlen (src)));
...
get_user_pass_http (struct http_proxy_info *p, const bool force)
{
	if (!static_proxy_user_pass.defined || force)
	{
		...
		get_user_pass (&static_proxy_user_pass, p->options.auth_file, UP_TYPE_PROXY, flags);
...
static void
dump_residual (socket_descriptor_t sd, int timeout, volatile int *signal_received)
{
	char buf[256];
	while (true)
	{
 ...
 */
static int
get_proxy_authenticate (socket_descriptor_t sd, int timeout, char **data, struct gc_arena *gc,
		...
		volatile int *signal_received)
{
	...
	else
	{
		...
		{
			openvpn_snprintf (buf, sizeof (buf), "User-Agent: %s", p->options.user_agent);
			if (!send_line_crlf (sd, buf))
				goto error;
		}
	...
	while (nparms >= 1 && status == 407)
	{
		...
		else if (p->auth_method == HTTP_AUTH_DIGEST && !processed)
		{
			...
			if (method == HTTP_AUTH_DIGEST)
			{
				...
				openvpn_snprintf (uri, sizeof (uri), "%s:%d", host, port);
				...
				DigestCalcResponse (session_key, nonce, nonce_count, (char *) cnonce,
					qop, http_method, uri, NULL, response);
		...
		else if (p->auth_method == HTTP_AUTH_NONE && p->options.auth_retry)
		{
			...
			char *pa = NULL;
			const int method = get_proxy_authenticate (sd, p->options.timeout, &pa, NULL, signal_received);
