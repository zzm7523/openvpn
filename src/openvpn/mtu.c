...
void
alloc_buf_sock_tun (struct buffer *buf, const struct frame *frame, const bool tuntap_buffer, const unsigned int align_mask)
{
...
frame_set_mtu_dynamic (struct frame *frame, int mtu, unsigned int flags)
{
	...
	if (!(flags & SET_MTU_UPPER_BOUND) || mtu < frame->link_mtu_dynamic)
	{
		frame->link_mtu_dynamic =
			constrain_int (mtu, EXPANDED_SIZE_MIN (frame), EXPANDED_SIZE (frame));
	}

	dmsg (D_MTU_DEBUG, "MTU DYNAMIC mtu=%d, flags=%u, %d -> %d",
		orig_mtu, flags, orig_link_mtu_dynamic, frame->link_mtu_dynamic);
}
...
void
set_mtu_discover_type (socket_descriptor_t sd, int mtu_type)
{
	if (mtu_type >= 0)
	{
#if defined(HAVE_SETSOCKOPT) && defined(SOL_IP) && defined(IP_MTU_DISCOVER)
		if (setsockopt (sd, SOL_IP, IP_MTU_DISCOVER, (const char *) &mtu_type, sizeof (mtu_type)))
