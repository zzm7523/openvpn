 ...
 *
 * We use the "sliding-window" algorithm, similar to IPSec.
 */
...
/*
 * Special time_t value that indicates that sequence number has expired.
 */
#define SEQ_UNSEEN  ((time_t) 0)
#define SEQ_EXPIRED ((time_t) 1)

static void 
packet_id_debug_print (int msglevel, const struct packet_id_rec *p, time_t now_sec, const struct packet_id_net *pin,
		const char *message, int value);

static inline void
packet_id_debug (int msglevel, const struct packet_id_rec *p, time_t now_sec, const struct packet_id_net *pin,
		const char *message, int value)
{
#ifdef ENABLE_DEBUG
	if (unlikely (check_debug_level (msglevel)))
		packet_id_debug_print (msglevel, p, now_sec, pin, message, value);
#endif
}

void
packet_id_init (struct packet_id *p, bool tcp_mode, int seq_backtrack, int time_backtrack,
		const char *name, int unit)
{
	dmsg (D_PID_DEBUG, "PID packet_id_init tcp_mode=%d seq_backtrack=%d time_backtrack=%d",
		tcp_mode, seq_backtrack, time_backtrack);
...
packet_id_free (struct packet_id *p)
{
	if (p)
	{
		...
		if (p->rec.seq_list)
			CIRC_LIST_FREE (p->rec.seq_list);
		CLEAR (*p);
	}
}

void
packet_id_add (struct packet_id_rec *p, time_t now_sec, const struct packet_id_net *pin)
{
	...
	if (p->seq_list)
	{
		...
		diff = p->id - pin->id;
		if (diff < (packet_id_type) CIRC_LIST_SIZE (p->seq_list) && local_now > SEQ_EXPIRED)
 ...
 * be accepted because they would violate time_backtrack.
 */
void
packet_id_reap (struct packet_id_rec *p, time_t now_sec)
{
	const time_t local_now = now_sec;
...
/*
 * Return true if packet id is ok, or false if it is a replay.
 */
bool
packet_id_test (struct packet_id_rec *p, time_t now_sec, const struct packet_id_net *pin)
{
	packet_id_type diff;

	packet_id_debug (D_PID_DEBUG, p, now_sec, pin, "PID_TEST", 0);
	...
	if (p->seq_backtrack)
	{
		...
		if (pin->time == p->time)
		{
			...
			if ((int) diff > p->max_backtrack_stat)
			{
				...
				packet_id_debug (D_PID_DEBUG_LOW, p, now_sec, pin, "PID_ERR replay-window backtrack occurred",
					p->max_backtrack_stat);
			}

			if (diff >= (packet_id_type) CIRC_LIST_SIZE (p->seq_list))
			{
				packet_id_debug (D_PID_DEBUG_LOW, p, now_sec, pin, "PID_ERR large diff", diff);
				return false;
			}

			{
				...
				else
				{
					...
					packet_id_debug (D_PID_DEBUG_MEDIUM, p, now_sec, pin, "PID_ERR replay", diff);
					return false;
				}
			}
		}
		...
		{
			packet_id_debug (D_PID_DEBUG_LOW, p, now_sec, pin, "PID_ERR time backtrack", 0);
			return false;
		}
	...
	if (!packet_id_persist_enabled (p))
	{
		...
		if (p->fd == -1)
		{
			msg (D_PID_PERSIST | M_ERRNO,
				"Cannot open --replay-persist file %s for read/write", filename);
		}
		else
		{
...
packet_id_persist_save (struct packet_id_persist *p)
{
		...
		p->id != p->id_last_written))
	{
		...
		if (seek_ret == (off_t) 0)
		{
			...
			{
				msg (D_PID_PERSIST | M_ERRNO,
					"Cannot write to --replay-persist file %s", p->filename);
			}
		}
		else
		{
			msg (D_PID_PERSIST | M_ERRNO,
				"Cannot seek to beginning of --replay-persist file %s", p->filename);
		}
		gc_free (&gc);
	}
}
	...
	if (packet_id_persist_enabled (p))
	{
		...
		if (p->time)
			buf_printf (&out, " / time = (" packet_id_format ") %s",
				(packet_id_print_type) p->time, time_string (p->time, 0, false, gc));
	}
		...
		const char *message, int value)
{
	...
	int i;
	const time_t prev_now = now_sec;

	CLEAR (tv);
	gettimeofday (&tv, NULL);	// 日志采用系统时间
	...
	buf_printf (&out, " r=[%d,%d,%d,%d,%d]", (int) (p->last_reap - tv.tv_sec), p->seq_backtrack,
		p->time_backtrack, p->max_backtrack_stat, (int) p->initialized);
...
packet_id_interactive_test (void)
{
	...
	const int thread_idx = get_thread_index (pthread_self ());
	...
	while (true)
	{
		...
		if (sscanf (buf, "%lu,%u", &pin.time, &pin.id) == 2)
		{
			...
			test = packet_id_test (&pid.rec, now_sec (thread_idx), &pin);
		...
		{
			long_form = (count < 20);
			packet_id_alloc_outgoing (&pid.send, &pin, long_form, now_sec (thread_idx));
