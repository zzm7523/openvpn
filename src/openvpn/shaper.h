...
/*
 * A simple traffic shaper for the output direction.
 */
...
shaper_delay (struct shaper *s)
{
	...
	{
		ASSERT (!openvpn_gettimeofday (&tv, NULL, MAIN_THREAD_INDEX));
...
shaper_wrote_bytes (struct shaper *s, int nbytes)
{
	...
	tv.tv_usec = s->bytes_per_second
		? min_int (max_int (nbytes, 100) * s->factor, (SHAPER_MAX_TIMEOUT * 1000000)) : 0;
#endif

	if (tv.tv_usec)
	{
		ASSERT (!openvpn_gettimeofday (&s->wakeup, NULL, MAIN_THREAD_INDEX));
		tv_add (&s->wakeup, &tv);
