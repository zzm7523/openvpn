...
#ifdef ENABLE_SSL
		, int current_cert_depth, openvpn_x509_cert_t *current_cert
#endif
);
...
struct openvpn_plugin_string_list* openvpn_plugin_string_list_find (struct openvpn_plugin_string_list *l, const char *name);
...
static inline void plugin_return_free (struct plugin_return *pr)
{
}
