		...
		volatile int *signal_received)
{
	struct addrinfo *ai;
	int status;
	status = openvpn_getaddrinfo (flags, hostname, resolve_retry_seconds, signal_received,
		AF_INET, &ai);
	if (status == 0)
	{
		struct in_addr ia;
 ...
 */
int
openvpn_getaddrinfo (unsigned int flags, const char *hostname, int resolve_retry_seconds, volatile int *signal_received,
		int ai_family, struct addrinfo **res)
{
	...
	/* getaddrinfo是同步阻塞的，直到返回成功或者失败。!!实测，成功时一般几十毫秒即可，失败时往往需要30秒以上。*/
	...
	if (status != 0) /* parse as numeric address failed? */
	{
		...
		msg (M_INFO, "RESOLVE_REMOTE: %s", hostname);
#ifdef ENABLE_MANAGEMENT
		if (flags & GETADDR_UPDATE_MANAGEMENT_STATE)
		{
			if (management)
				management_set_state (management, OPENVPN_STATE_RESOLVE, NULL, (in_addr_t) 0, (in_addr_t) 0);
		}
#endif
		...
		while (true)
		{
...
#ifdef WIN32
			{
				...
				ASSERT (is_main_thread ());	// gai_strerrorW is not thread safe
#endif
				error_data = gai_strerrorW (status);
				n = WideCharToMultiByte (CP_UTF8, 0, error_data, -1, NULL, 0, NULL, NULL);
				error_msg = (LPSTR) gc_malloc (n, false, &gc);
				WideCharToMultiByte (CP_UTF8, 0, error_data, -1, error_msg, n, NULL, NULL);
				msg (level, fmt, hostname, error_msg);
			}
#else
			msg (level, fmt, hostname, gai_strerror (status));
#endif
			if (--resolve_retries <= 0)
				goto done;
		...
		/* Do not chose an IP Addresse by random or change the order  of IP addresses,
		 * doing so will break RFC 3484 address selection *
		 */
	}
	else
	{
...
static void
update_remote (const char *host, struct openvpn_sockaddr *addr, bool *changed, const unsigned int sockflags)
{
...
socket_get_sndbuf (socket_descriptor_t sd)
{
	...
	len = sizeof (val);
	if (getsockopt (sd, SOL_SOCKET, SO_SNDBUF, (char *) &val, &len) == 0 && len == sizeof (val))
		return val;
#endif
...
socket_get_rcvbuf (socket_descriptor_t sd)
{
	...
	len = sizeof (val);
	if (getsockopt (sd, SOL_SOCKET, SO_RCVBUF, (char *) &val, &len) == 0 && len == sizeof (val))
		return val;
#endif
...
socket_set_tcp_nodelay (socket_descriptor_t sd, int state)
{
	...
	if (setsockopt (sd, IPPROTO_TCP, TCP_NODELAY, (char *) &state, sizeof (state)) != 0)
	{
...
/*
 * SOCKET INITALIZATION CODE. Create a TCP/UDP socket
 */
...
static void
socket_do_listen (socket_descriptor_t sd, const struct openvpn_sockaddr *local, bool do_listen,
...
socket_listen_accept (socket_descriptor_t sd, struct link_socket_actual *act, const char *remote_dynamic,
		bool *remote_changed, const struct openvpn_sockaddr *local, bool do_listen, bool nowait,
		volatile int *signal_received)
{
	struct gc_arena gc = gc_new ();
	...
	while (true)
	{
...
#else
		status = select (0, &reads, NULL, NULL, &tv);
#endif
		get_signal (signal_received);
		if (*signal_received)
		{
		...
		if (socket_defined (new_sd))
		{
			...
			if (addr_defined (&remote_verify) && !addr_match (&remote_verify, &act->dest))
			{
...
void
socket_bind (socket_descriptor_t sd, struct openvpn_sockaddr *local, const char *prefix)
{
	struct gc_arena gc = gc_new ();
...
int
openvpn_connect (socket_descriptor_t sd, struct openvpn_sockaddr *remote, int connect_timeout,
		volatile int *signal_received)
{
	int status = 0;
		...
		)
	{
		while (true)
		{
...
#else
			status = select (0, NULL, &writes, NULL, &tv);
#endif
#endif
			if (signal_received)
			{
			...
			/* got it */
			{
				...
				len = sizeof (val);
				if (getsockopt (sd, SOL_SOCKET, SO_ERROR, (char *) &val, &len) == 0 && len == sizeof (val))
					status = val;
				else
					status = openvpn_errno ();
				break;
...
static void
socket_connect (socket_descriptor_t *sd, struct openvpn_sockaddr *local, bool bind_local,
		struct openvpn_sockaddr *remote, const bool connection_profiles_defined,
		const char *remote_dynamic, bool *remote_changed, const int connect_retry_seconds,
		const int connect_timeout, const int connect_retry_max, const unsigned int sockflags,
		volatile int *signal_received)
{
	struct gc_arena gc = gc_new ();
	int retry = 0;
	...
	while (true)
	{
...
#ifdef ENABLE_MANAGEMENT
		if (management)
			management_set_state (management, OPENVPN_STATE_TCP_CONNECT,
				NULL, (in_addr_t) 0, (in_addr_t) 0);
#endif
		...
		msg (D_LINK_ERRORS, "TCP: connect to %s failed, will try again in %d seconds: %s",
			print_sockaddr (remote, &gc),
	...
	msg (M_INFO, "TCP connection established with %s", print_sockaddr (remote, &gc));
...
socket_frame_init (const struct frame *frame, struct link_socket *sock)
{
	...
	ASSERT (MAX_OVERLAPPED_SIZE >= DEF_OVERLAPPED_WRITE && DEF_OVERLAPPED_WRITE >= 2);
	/*
	 ...
	 */
	ASSERT (PKTINFO_BUF_SIZE >= CMSG_SPACE (sizeof (struct in6_pktinfo)) &&
		PKTINFO_BUF_SIZE >= CMSG_SPACE (sizeof (struct in_pktinfo)) &&
		PKTINFO_BUF_SIZE >= CMSG_SPACE (sizeof (struct in_addr)));
	...
	for (i = 0; i < MAX_OVERLAPPED_SIZE; ++i)
	{
		...
		ASSERT (buf_init (&buf, FRAME_HEADROOM_ADJ (frame, FRAME_HEADROOM_MARKER_READ_LINK)));
		sock->reads.items[i].buf = buf;

		sock->reads.iov[i].iov_base = BPTR (&sock->reads.items[i].buf);
		sock->reads.iov[i].iov_len = buf_forward_capacity_total (&sock->reads.items[i].buf);
		sock->reads.mesg[i].msg_len = 0;
		sock->reads.mesg[i].msg_hdr.msg_iov = &sock->reads.iov[i];
		sock->reads.mesg[i].msg_hdr.msg_iovlen = 1;
		sock->reads.mesg[i].msg_hdr.msg_name = &sock->reads.items[i].addr.dest.addr;
		sock->reads.mesg[i].msg_hdr.msg_namelen = sizeof (sock->reads.items[i].addr.dest.addr);
		sock->reads.mesg[i].msg_hdr.msg_control = sock->reads.pktinfo_buf[i];
		sock->reads.mesg[i].msg_hdr.msg_controllen = sizeof (sock->reads.pktinfo_buf[i]);

		buf = alloc_buf (BUF_SIZE (frame));
		ASSERT (buf_init (&buf, FRAME_HEADROOM_ADJ (frame, FRAME_HEADROOM_MARKER_READ_LINK)));
		sock->writes.items[i].buf = buf;

		sock->writes.iov[i].iov_base = BPTR (&sock->writes.items[i].buf);
		sock->writes.iov[i].iov_len = buf_forward_capacity_total (&sock->writes.items[i].buf);
		sock->writes.mesg[i].msg_len = 0;
		sock->writes.mesg[i].msg_hdr.msg_iov = &sock->writes.iov[i];
		sock->writes.mesg[i].msg_hdr.msg_iovlen = 1;
		sock->writes.mesg[i].msg_hdr.msg_name = &sock->writes.items[i].addr.dest.addr;
		sock->writes.mesg[i].msg_hdr.msg_namelen = sizeof (sock->writes.items[i].addr.dest.addr);
		sock->writes.mesg[i].msg_hdr.msg_control = sock->writes.pktinfo_buf[i];
		sock->writes.mesg[i].msg_hdr.msg_controllen = sizeof (sock->writes.pktinfo_buf[i]);
	}
#else
#ifdef WIN32
...
/*
 * Adjust frame structure based on a Path MTU value given to us by the OS.
 */
void
		...
		case AF_INET6:
			{
				...
				if (status != 0)
				{
					msg (M_FATAL, "getaddr6() failed for local \"%s\": %s", sock->local_host, gai_strerror (status));
				}
	...
	if (!sock->did_resolve_remote)
	{
		...
		if (!addr_defined (&sock->info.lsa->remote))
		{
			...
			if (sock->remote_host)
			{
				...
				status = openvpn_getaddrinfo (flags, sock->remote_host, retry, signal_received, af, &ai);
				if (status == 0)
				{
		...
		unsigned int sockflags)
{
...
#ifdef ENABLE_MASQUERADE
	sock->masq_options = masq_options;
#endif
	...
	{
		ASSERT (accept_from);
		ASSERT (sock->info.proto == PROTO_TCPv4_SERVER || sock->info.proto == PROTO_TCPv6_SERVER);
		ASSERT (!sock->inetd);
	...
	if (sock->inetd)
	{
		ASSERT (sock->info.proto != PROTO_TCPv4_CLIENT && sock->info.proto != PROTO_TCPv6_CLIENT);
...
void
link_socket_init_phase2 (struct link_socket *sock, const struct frame *frame, volatile int *signal_received)
{
	struct gc_arena gc = gc_new ();
	...
	/*
	* Pass a remote name to connect/accept so that they can test for
	* dynamic IP address changes and throw a SIGUSR1 if appropriate.
	*/
	if (sock->resolve_retry_seconds)
...
#ifdef HAVE_GETSOCKNAME
			{
				...
				if (getsockname (sock->sd, (struct sockaddr *) &local_addr, &addrlen) == 0)
				{
					...
					dmsg (D_SOCKET_DEBUG, "inetd(%s): using sa_family=%d from getsockname(%d)",
						proto2ascii (sock->info.proto, false), local_addr.addr.sa.sa_family, sock->sd);
				}
				else
...
#else
			msg (M_WARN, "inetd(%s): this OS does not provide the getsockname() function, using AF_INET",
	...
	else
	{
		...
		if (sock->info.proto == PROTO_TCPv4_SERVER ||sock->info.proto == PROTO_TCPv6_SERVER)
		{
			...
			case LS_MODE_TCP_LISTEN:
				socket_do_listen (sock->sd, &sock->info.lsa->local, true, false);
				break;
	...
	/* print local address */
	{
		...
		if (sock->inetd)
			...
			msg (msglevel, "%s link local%s: %s", proto2ascii (sock->info.proto, true),
				(sock->bind_local ? " (bound)" : ""),
				print_sockaddr_ex (&sock->info.lsa->local, ":", sock->bind_local ? PS_SHOW_PORT : 0, &gc));

		/* print active remote address */
		msg (msglevel, "%s link remote: %s", proto2ascii (sock->info.proto, true),
	...
	if (sock)
	{
		...
		if (socket_defined (sock->sd))
		{
			...
			if (!gremlin)
			{
				...
				for (i = 0; i < MAX_OVERLAPPED_SIZE; ++i)
				{
					free_buf (&sock->reads.items[i].buf);
					free_buf (&sock->writes.items[i].buf);
				}
#else
#ifdef WIN32
...
ipchange_fmt (const bool include_cmd, struct argv *argv, const struct link_socket_info *info, struct gc_arena *gc)
{
	...
	if (include_cmd)
		argv_printf (argv, "%sc %s %s", info->ipchange_command, ip, port);
	else
		argv_printf (argv, "%s %s", ip, port);
}
		...
		const struct link_socket_actual *from_addr)
{
	...
	buf->len = 0;
	buf_set_tracking (buf, PACKET_DROP_BAD_SOURCE_ADDRESS);
...
/*
 * Stream buffer functions, used to packetize a TCP stream connection.
 */
		...
		)
{
	...
	/* if length unknown, see if we can get the length prefix from the head of the buffer */
	if (sb->len < 0 && sb->buf.len >= (int) sizeof (packet_size_type))
	{
		...
		if (sb->port_share_state == PS_ENABLED)
		{
			...
			{
#ifdef _DEBUG
				msg (M_INFO, "OpenVPN client protocol detected");
#endif
...
/*
 * The listen event is a special event whose sole purpose is to tell us that there's 
 * a new incoming connection on a TCP socket, for use in server mode.
 */
event_t
...
const char *
print_sockaddr_ex (const struct openvpn_sockaddr *addr, const char *separator, const unsigned int flags,
		struct gc_arena *gc)
{
	...
	case AF_INET:
		{
			...
			if (((flags & PS_SHOW_PORT) || (addr_defined (addr) && (flags & PS_SHOW_PORT_IF_DEFINED))) && port)
			{
				if (separator)
	...
	case AF_INET6:
		{
			...
			if (((flags & PS_SHOW_PORT) || (addr_is_defined && (flags & PS_SHOW_PORT_IF_DEFINED))) && port)
			{
				if (separator)
...
const char *
print_link_socket_actual_ex (const struct link_socket_actual *act, const char *separator, const unsigned int flags,
		struct gc_arena *gc)
{
	if (act)
	{
...
#error ENABLE_IP_PKTINFO is set without IP_PKTINFO xor IP_RECVDSTADDR (fix syshead.h)
					...
					buf_printf (&out, " (via %s%%%s)",
						print_sockaddr_ex (&sa, separator, 0, gc), ifname);
				}
				break;
			case AF_INET6:
				{
...
/*
 * Convert an in_addr_t in host byte order to an ascii dotted quad.
 */
const char *
...
/*
 * Convert an in6_addr in host byte order to an ascii representation of an IPv6 address
 */
const char *
...
/* add some offset to an ipv6 address (add in steps of 8 bits, taking overflow into next round) */
struct in6_addr 
...
void
setenv_in6_addr (struct env_set *es, const char *name_prefix, const struct in6_addr *addr, const unsigned int flags)
{
...
void
setenv_link_socket_actual (struct env_set *es, const char *name_prefix, const struct link_socket_actual *act,
		const unsigned int flags)
{
...
/*
 * Given a local proto, return local proto if !remote, 
 * or compatible remote proto if remote.
 *
 * This is used for options compatibility checking.
 *
...
/*
 * Bad incoming address lengths that differ from what we expect are considered to be fatal errors.
 */
void
...
int
link_socket_read_tcp (struct link_socket *sock, struct buffer *buf, unsigned int flags)
{
	int len = 0;
	...
	if (sock->stream_buf.residual_fully_formed || stream_buf_added (&sock->stream_buf, len
#ifdef ENABLE_MASQUERADE
			, sock->masq_options
#endif
		)) /* packet complete? */
...
link_socket_read_udp_posix_recvmsg (struct link_socket *sock, struct buffer *buf,
		struct link_socket_actual *from, unsigned int flags)
{
	struct iovec iov;
...
static socklen_t
link_socket_read_udp_posix_recvmmsg (struct link_socket *sock, struct buffer *buf,
		struct link_socket_actual *from, unsigned int flags)
{
	int ret = 0;
	socklen_t fromlen = sizeof (from->dest.addr);

	if ((sock->reads.size == 0 || sock->reads.offset >= sock->reads.size) &&
			!(flags & OVERLAPPED_READ_EMPTY_RETURN))
	{
		...
		ret = recvmmsg (sock->sd, sock->reads.mesg, MAX_OVERLAPPED_SIZE, 0, NULL);
		if (ret > 0)
		{
			++sock->reads.stats[ret];
			sock->reads.size = ret;

			for (i = 0; i < sock->reads.size; ++i)
			{
				sock->reads.items[i].buf.len = sock->reads.mesg[i].msg_len;
				sock->reads.items[i].addrlen = sock->reads.mesg[i].msg_hdr.msg_namelen;
				cmsg = CMSG_FIRSTHDR (&sock->reads.mesg[i].msg_hdr);
				if (cmsg != NULL
					&& CMSG_NXTHDR (&sock->reads.mesg[i].msg_hdr, cmsg) == NULL
#ifdef IP_PKTINFO
					&& cmsg->cmsg_level == SOL_IP 
					&& cmsg->cmsg_type == IP_PKTINFO
					&& cmsg->cmsg_len >= CMSG_LEN (sizeof (struct in_pktinfo)))
...
#error ENABLE_IP_PKTINFO is set without IP_PKTINFO xor IP_RECVDSTADDR (fix syshead.h)
#endif
				{
					...
					sock->reads.items[i].addr.pi.in4.ipi_ifindex = pkti->ipi_ifindex;
					sock->reads.items[i].addr.pi.in4.ipi_spec_dst = pkti->ipi_spec_dst;
#elif defined(IP_RECVDSTADDR)
					sock->reads.items[i].addr.pi.in4 = *(struct in_addr*) CMSG_DATA (cmsg);
#else
#error ENABLE_IP_PKTINFO is set without IP_PKTINFO xor IP_RECVDSTADDR (fix syshead.h)
#endif
				}
					...
					&& cmsg->cmsg_len >= CMSG_LEN (sizeof (struct in6_pktinfo)))
				{
					...
					sock->reads.items[i].addr.pi.in6.ipi6_ifindex = pkti6->ipi6_ifindex;
					sock->reads.items[i].addr.pi.in6.ipi6_addr = pkti6->ipi6_addr;
				}
			}
		}
	}
	...
	if (sock->reads.size > 0 && sock->reads.offset < sock->reads.size)
	{
		...
		*buf = sock->reads.items[sock->reads.offset].buf;
		sock->reads.items[sock->reads.offset].buf.data = data;
		*from = sock->reads.items[sock->reads.offset].addr;
		fromlen = sock->reads.items[sock->reads.offset].addrlen;
		
		sock->reads.iov[sock->reads.offset].iov_base = BPTR (&sock->reads.items[sock->reads.offset].buf);
		sock->reads.mesg[sock->reads.offset].msg_len = 0;
		sock->reads.mesg[sock->reads.offset].msg_hdr.msg_namelen =
			sizeof (sock->reads.items[sock->reads.offset].addr.dest.addr);
		++sock->reads.offset;
	}
	else
		buf->len = ret;	/* 未读到数据或发生错误, 必须设置为0或-1 */
...
int
link_socket_read_udp_posix (struct link_socket *sock, struct buffer *buf, struct link_socket_actual *from, unsigned int flags)
{
	socklen_t fromlen = sizeof (from->dest.addr);
	...
	if (sock->sockflags & SF_USE_RECVMMSG)
		fromlen = link_socket_read_udp_posix_recvmmsg (sock, buf, from, flags);
	else
#endif
	{
#if ENABLE_IP_PKTINFO
		if (sock->sockflags & SF_USE_IP_PKTINFO)
			fromlen = link_socket_read_udp_posix_recvmsg (sock, buf, from, flags);
		else
#endif
...
int
link_socket_write_tcp (struct link_socket *sock, struct buffer *buf, struct link_socket_actual *to, unsigned int flags)
{
#ifdef WIN32
	return link_socket_write_win32 (sock, buf, to, flags);
#else
	return link_socket_write_tcp_posix (sock, buf, to, flags);  
#endif
}
...
#if defined(TARGET_LINUX) || defined(TARGET_ANDROID)
int
link_socket_write_udp_posix_sendmmsg (struct link_socket *sock, struct buffer *buf,
		struct link_socket_actual *to, unsigned int flags)
{
	int status = buf->len;	/* 记住当前包长 */

	ASSERT (buf->len > 0);

	if (flags & OVERLAPPED_PACKET_INVALID)
	{
		buf->len = 0;	/* 用来刷新缓存的无效包, 置零释放 */
	}

	if (buf->len > 0)
	{
		if (sock->writes.size < MAX_OVERLAPPED_SIZE)
		{
			uint8_t *data = sock->writes.items[sock->writes.size].buf.data;
			sock->writes.items[sock->writes.size].buf = *buf;
			buf->data = data;
			buf->len = 0;	/* 包已缓冲 */
			sock->writes.items[sock->writes.size].addr = *to;
			++sock->writes.size;
		}
		else
			status = 0;	/* 发送缓冲不足, 稍后重发包 */
	}

	/* 强制刷新或缓存区已满 */
	if ((flags & OVERLAPPED_FORCE_FLUSH) || status == 0)
	{
		int i, ret, vlen = sock->writes.size - sock->writes.offset;
		struct cmsghdr *cmsg;
		struct in_pktinfo *pkti;
		struct in6_pktinfo *pkti6;

		for (i = sock->writes.offset; i < sock->writes.size; ++i)
		{
			sock->writes.iov[i].iov_base = BPTR (&sock->writes.items[i].buf);
			sock->writes.iov[i].iov_len = BLEN (&sock->writes.items[i].buf);
			sock->writes.mesg[i].msg_len = 0;
			...
			case AF_INET:
				{
					sock->writes.mesg[i].msg_hdr.msg_name = &sock->writes.items[i].addr.dest.addr.sa;
					sock->writes.mesg[i].msg_hdr.msg_namelen = sizeof (struct sockaddr_in);
					sock->writes.mesg[i].msg_hdr.msg_flags = 0;
#ifdef HAVE_IN_PKTINFO
					sock->writes.mesg[i].msg_hdr.msg_controllen = CMSG_SPACE (sizeof (struct in_pktinfo));
					cmsg = CMSG_FIRSTHDR (&sock->writes.mesg[i].msg_hdr);
					cmsg->cmsg_len = CMSG_LEN (sizeof (struct in_pktinfo));
					cmsg->cmsg_level = SOL_IP;
					cmsg->cmsg_type = IP_PKTINFO;
					pkti = (struct in_pktinfo *) CMSG_DATA (cmsg);
					pkti->ipi_ifindex = sock->writes.items[i].addr.pi.in4.ipi_ifindex;
					pkti->ipi_spec_dst = sock->writes.items[i].addr.pi.in4.ipi_spec_dst;
					pkti->ipi_addr.s_addr = 0;
#elif defined(IP_RECVDSTADDR)
					ASSERT (CMSG_SPACE (sizeof (struct in_addr)) <= sizeof (sock->writes.pktinfo_buf[i]));
					sock->writes.mesg[i].msg_hdr.msg_controllen = CMSG_SPACE (sizeof (struct in_addr));
					cmsg = CMSG_FIRSTHDR (&sock->writes.mesg[i].msg_hdr);
					cmsg->cmsg_len = CMSG_LEN (sizeof (struct in_addr));
					cmsg->cmsg_level = IPPROTO_IP;
					cmsg->cmsg_type = IP_RECVDSTADDR;
					*(struct in_addr *) CMSG_DATA (cmsg) = sock->writes.items[i].addr.pi.in4;
#else
#error ENABLE_IP_PKTINFO is set without IP_PKTINFO xor IP_RECVDSTADDR (fix syshead.h)
#endif
					break;
			...
			case AF_INET6:
				{
					sock->writes.mesg[i].msg_hdr.msg_name = &sock->writes.items[i].addr.dest.addr.sa;
					sock->writes.mesg[i].msg_hdr.msg_namelen = sizeof (struct sockaddr_in6);
					ASSERT (CMSG_SPACE (sizeof (struct in6_pktinfo)) <= sizeof (sock->writes.pktinfo_buf[i]));
					sock->writes.mesg[i].msg_hdr.msg_controllen = CMSG_SPACE (sizeof (struct in6_pktinfo));
					sock->writes.mesg[i].msg_hdr.msg_flags = 0;
					cmsg = CMSG_FIRSTHDR (&sock->writes.mesg[i].msg_hdr);
					cmsg->cmsg_len = CMSG_LEN (sizeof (struct in6_pktinfo));
					cmsg->cmsg_level = IPPROTO_IPV6;
					cmsg->cmsg_type = IPV6_PKTINFO;
					pkti6 = (struct in6_pktinfo *) CMSG_DATA (cmsg);
					pkti6->ipi6_ifindex = sock->writes.items[i].addr.pi.in6.ipi6_ifindex;
					pkti6->ipi6_addr = sock->writes.items[i].addr.pi.in6.ipi6_addr;
					break;
				}
			default:
				ASSERT (0);
		...
		ret = sendmmsg (sock->sd, &sock->writes.mesg[sock->writes.offset], vlen, 0);
		if (ret > 0)
		{
			++sock->writes.stats[ret];

			if (ret == vlen)	/* 全部写出成功 */
			{
				sock->writes.offset = 0;
				sock->writes.size = 0;
			}
			else	/* 部分写出成功 */
			{
				sock->writes.offset += ret;
#ifdef _DEBUG
				ASSERT (sock->writes.offset < sock->writes.size);
#endif
			}
		}
		else if (ret < 0)	/* 写出失败 */
		{
			...
			if (my_errno == EAGAIN || my_errno == EWOULDBLOCK)
				status = 0;	/* 发送缓冲不足, 包已经缓存在sock->writes */
			else
			{
...
int
socket_recv_queue (struct link_socket *sock, int maxsize, struct buffer *buf, struct link_socket_actual *from)
{
	if (sock->reads.iostate == IOSTATE_INITIAL)
	{
		...
		{
			ASSERT (sock->stream_buf.residual_fully_formed == 0);
		...
		wsabuf[0].buf = (CHAR *) BPTR (&sock->reads.buf);
		wsabuf[0].len = maxsize > 0 ? maxsize : BLEN (&sock->reads.buf);

		/* check for buffer overflow */
		ASSERT (wsabuf[0].len <= (ULONG) BLEN (&sock->reads.buf));
		...
		if (!status) /* operation completed immediately? */
		{
			...
			if (buf)
			{
				sock->reads.iostate = IOSTATE_INITIAL;
				buf_copy_n (buf, &sock->reads.buf, sock->reads.size);
			}
			else
			{
			...
			if (from)
			{
				if (sock->reads.size >= 0 && sock->reads.addr_defined)
				{
					 ...
					 */
					switch (sock->reads.addrlen)
					{
					case sizeof (struct sockaddr_in):
					case sizeof (struct sockaddr_in6):
						/* TODO(jjo): for some reason (?) I'm getting 24,28 for AF_INET6 under WIN32*/
					case sizeof (struct sockaddr_in6)-4:
						break;
					default:
						bad_address_length (sock->reads.addrlen, af_addr_size (sock->reads.addr.sin_family));
					}

					switch (sock->reads.addr.sin_family)
					{
					case AF_INET:
						from->dest.addr.in4 = sock->reads.addr;
						break;
					case AF_INET6:
						from->dest.addr.in6 = sock->reads.addr6;
						break;
					}
				}
				else
		...
		else
		{
			...
			else /* error occurred */
			{
				...
				dmsg (D_WIN32_IO, "WIN32 I/O: Socket Receive error [%d]: %s",
					(int) wsabuf[0].len, strerror_win32 (status, &gc));
				gc_free (&gc);
			}
		}
	}
...
socket_send_queue (struct link_socket *sock, struct buffer *buf, const struct link_socket_actual *to)
{
	if (sock->writes.iostate == IOSTATE_INITIAL)
	{
		...
		/* Win32 docs say it's okay to allocate the wsabuf on the stack */
		wsabuf[0].buf = (CHAR *) BPTR (&sock->writes.buf);
		...
		if (!status) /* operation completed immediately? */
		{
			sock->writes.iostate = IOSTATE_INITIAL;
			...
			dmsg (D_WIN32_IO, "WIN32 I/O: Socket Send immediate return [%d,%d]",
				(int) wsabuf[0].len, (int) sock->writes.size);	       
		}
		else
		{
			status = WSAGetLastError (); 
...
int
socket_finalize (SOCKET s, struct overlapped_io *io, struct buffer *buf, struct link_socket_actual *from)
{
	int ret = -1;
	BOOL status;
	...
	case IOSTATE_QUEUED:
		...
		{
			if (GetLastError() == WSA_IO_INCOMPLETE) /* 被event_wakeup唤醒 */
			{
				ret = 0;
...
socket_set (struct link_socket *s, struct event_set *es, unsigned int rwflags, void *arg, unsigned int *persistent)
{
	...
	{
		if (rwflags & EVENT_READ)
			ASSERT (stream_buf_read_setup (s));

#ifdef WIN32
		if (rwflags & EVENT_READ)
			socket_recv_queue (s, 0, NULL, NULL);
#endif
