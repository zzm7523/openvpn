...
/*
 * Alignment of payload data such as IP packet or ethernet frame.
 */
...
void set_mtu_discover_type (socket_descriptor_t sd, int mtu_type);
...
static inline void
frame_dec_from_extra_frame (struct frame *frame, const int decrease)
{
	frame->extra_frame -= decrease;
}
