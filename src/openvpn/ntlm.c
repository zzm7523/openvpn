...
gen_md4_hash (const char *data, int data_len, char *result)
{
	...
	md_full (md4_kt, (const uint8_t *) data, data_len, (uint8_t *) md);
...
{ 
	...
	timestamp_ull = openvpn_time (NULL, MAIN_THREAD_INDEX);
...
ntlm_phase_3 (const struct http_proxy_info *p, const char *phase_2, struct gc_arena *gc)
{
	...
	/* username parsing */
	separator = (char *) strchr (p->up.username, '\\');
	if (separator == NULL)
	{
	...
	/* we can be sure that phase_2 is less than 128 therefore buf2 needs to be (3/4 * 128) */
	...
	if (ntlmv2_enabled)
	{
		...
		gen_hmac_md5 (userdomain_u, 2 * (int) strlen (userdomain), md4_hash, MD5_DIGEST_LENGTH, ntlmv2_hash);
	...
	else
	{
		...
		cipher_des_encrypt_ecb (key1, (unsigned char *) challenge, (unsigned char *) ntlm_response);

		create_des_keys ((unsigned char *) &(md4_hash[DES_KEY_LENGTH-1]), key2);
		cipher_des_encrypt_ecb (key2, (unsigned char *) challenge, (unsigned char *) &ntlm_response[DES_KEY_LENGTH]);

		create_des_keys ((unsigned char *) &(md4_hash[2*(DES_KEY_LENGTH-1)]), key3);
		cipher_des_encrypt_ecb (key3, (unsigned char *) challenge, (unsigned char *) &ntlm_response[DES_KEY_LENGTH*2]);
	}
	...
	/* username in ascii */
	add_security_buffer (0x24, username, (int) strlen (username), phase3, &phase3_bufpos);

	/* Set domain. If <domain> is empty, default domain will be used (i.e. proxy's domain) */ 
	add_security_buffer (0x1c, domain, (int) strlen (domain), phase3, &phase3_bufpos);
