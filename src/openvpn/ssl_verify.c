	...
	if (multi->locked_username)
	{
		if (!username || strcmp (username, multi->locked_username))
		{
			msg (D_TLS_ERRORS, "TLS Auth Error: username attempted to change from '%s' to '%s' -- tunnel disabled",
				multi->locked_username, np (username));
		...
		const char *subject, const char *common_name)
{
	...
	if (opt->verify_x509_type != VERIFY_X509_NONE)
	{
		...
		{
			msg (D_HANDSHAKE, "VERIFY X509NAME ERROR: %s, must be %s", subject, opt->verify_x509_name);
			return FAILURE;		/* Reject connection */
		}
	}
		...
		)
{
	...
	/* export X509 cert SHA1 fingerprint */
	{
		...
		setenv_str (es, envname, format_hex_ex (sha1_hash, SHA_DIGEST_LENGTH, 0, 1, ":", &gc));
	}
...
verify_cert_call_command (const char *verify_command, struct env_set *es, int cert_depth,
		openvpn_x509_cert_t *cert, char *subject, const char *verify_export_cert)
{
...
result_t
verify_cert (struct tls_session *session, openvpn_x509_cert_t *cert, int cert_depth, X509_STORE_CTX *ctx)
{
	result_t ret = FAILURE;
	char *subject = NULL;
	...
	if (!subject)
	{
		msg (D_TLS_ERRORS, "VERIFY ERROR: depth=%d, could not extract X509 subject string from certificate",
			cert_depth);
		goto cleanup;
	}
	...
	if (SUCCESS != backend_x509_get_username (common_name, sizeof (common_name), opt->x509_username_field, cert))
	{
		if (!cert_depth)
		{
			msg (D_TLS_ERRORS, "VERIFY ERROR: could not extract %s from X509 "
				"subject string ('%s') -- note that the username length is limited to %d characters",
	...
	{
		msg (D_TLS_ERRORS, "TLS Error: Convoluted certificate chain detected with depth [%d] greater than %d",
	...
	{
#ifdef ENABLE_GUOMI
		if (GM_X509_get_usage (cert) == GM_X509_USAGE_SIGN)
#endif
...
/*
 * Return current session authentication state.  Return value is TLS_AUTHENTICATION_x.
 */
...
#ifdef ENABLE_DEF_AUTH
		if (latency && multi->tas_last && multi->tas_last + latency >= now_sec (MAIN_THREAD_INDEX))
			return TLS_AUTHENTICATION_UNDEFINED;
		multi->tas_last = now_sec (MAIN_THREAD_INDEX);
#endif /* ENABLE_DEF_AUTH */
			...
			if (DECRYPT_KEY_ENABLED (multi, ks))
			{
				...
				if (ks->authenticated)
				{
					...
					case ACF_DISABLED:
						...
						break;
					case ACF_UNDEFINED:
						if (now_sec (MAIN_THREAD_INDEX) < ks->auth_deferred_expire)
							deferred = true;
						break;
					case ACF_FAILED:
	...
	if ((session->opt->ssl_flags & SSLF_AUTH_USER_PASS_OPTIONAL) || strlen (up->username))
	{
		...
		if (session->opt->auth_user_pass_verify_script_via_file)
		{
			...
			if (tmp_file)
			{
				...
				if (!status_close (so))
				{
					msg (D_TLS_ERRORS, "TLS Auth Error: could not write username/password to file: %s", tmp_file);
					goto done;
				}
			}
			else
			{
				msg (D_TLS_ERRORS, "TLS Auth Error: could not create write username/password to temp file");
			}
		}
		else
		{
		...
		/* call command */
		ret = openvpn_run_script (&argv, session->opt->es, 0, "--auth-user-pass-verify");
...
static void
verify_user_pass_plugin_post (struct tls_multi *multi, const struct peer_info *pi, struct plugin_return *pr)
{
#ifdef ENABLE_PLUGIN
	struct version_info vi = get_peer_version_info (pi);	/* 获取客户端版本 */

	if (vi.major >= 6)	/* 如果客户端版本大于等于6, 推送策略、资源到客户端 */
	{
		...
		if (plugin_return_defined (&config))
		{
			...
			for (i = 0; i < config.n; ++i)
			{
				if (config.list[i] && config.list[i]->value)
					options_string_import (&multi->c->options, config.list[i]->value, D_IMPORT_ERRORS|M_OPTERR,
						option_permissions_mask, &option_types_found, multi->c->c2.es);
			}
		}
	}
#endif
...
void
tls_multi_set_client_reason (struct tls_multi *multi, struct plugin_return *pr)
{
#ifdef ENABLE_PLUGIN
#define MAX_CLIENT_REASON_LENGTH	512
	int i;
	struct buffer buf = alloc_buf (MAX_CLIENT_REASON_LENGTH);
	struct openvpn_plugin_string_list *sl;

	for (i = 0; i < pr->n; ++i)
	{
		if (pr->list[i])
		{
			sl = openvpn_plugin_string_list_find (pr->list[i], "client-reason");
			if (sl)
				buf_printf (&buf, "%s %s\n", sl->name, sl->value);
		}
	}
	...
	if (BLEN (&buf) > 0)
		multi->client_reason = BSTR (&buf);	/* 设置客户认证失败理由 */
	else
		free_buf (&buf);
#endif
}
...
verify_user_pass_plugin (struct tls_multi *multi, struct tls_session *session, const struct user_pass *up,
		const struct peer_info *pi, const char *raw_username)
{
	...
	if ((session->opt->ssl_flags & SSLF_AUTH_USER_PASS_OPTIONAL) || strlen (up->username))
	{
		...
		if (pi)
		{
			setenv_str (session->opt->es, "ticket", get_peer_info_value (pi, "UV_TICKET"));
			setenv_str (session->opt->es, "fingerprint", get_peer_info_value (pi, "UV_FINGERPRINT"));	

			setenv_str (session->opt->es, "lang", get_peer_info_value (pi, "UV_LANG"));
			setenv_str (session->opt->es, "product_version", get_peer_info_value (pi, "IV_VER"));			
			setenv_str (session->opt->es, "platform", get_peer_info_value (pi, "IV_PLAT"));
			setenv_str (session->opt->es, "platform_version", get_peer_info_value (pi, "IV_PLAT_VER"));
		}
		...
		if (retval == OPENVPN_PLUGIN_FUNC_SUCCESS)
		{			
			verify_user_pass_plugin_post (multi, pi, &pr);
		}
		else if (retval == OPENVPN_PLUGIN_FUNC_ERROR)
		{
			tls_multi_set_client_reason (multi, &pr);
		}
		...
		setenv_del (session->opt->es, "ticket");
		setenv_del (session->opt->es, "password");
		if (raw_username)
			setenv_str (session->opt->es, "username", up->username);
		setenv_del (session->opt->es, "common_name");
	}
	else
	{
 ...
 */
void
verify_user_pass (struct user_pass *up, struct peer_info *pi, struct tls_multi *multi, struct tls_session *session)
{
	...
	*/
	...
	if (plugin_defined (session->opt->plugins, OPENVPN_PLUGIN_AUTH_USER_PASS_VERIFY))
		s1 = verify_user_pass_plugin (multi, session, up, pi, raw_username);
	...
	{
		msg (D_TLS_ERRORS, "TLS Auth Error: --username-as-common name specified and username is longer than"
			" the maximum permitted Common Name length of %d characters",
			TLS_USERNAME_LEN);
