 ...
 * 			OPENVPN_MODE_OFB or \c OPENVPN_MODE_CFB or OPENVPN_MODE_CTR
 */
...
/**
 * Check if the supplied cipher is a supported OFB or CFB or CTR mode cipher.
 *
 * @param cipher	Static cipher parameters.
 *
 * @return		true iff the cipher is a OFB or CFB or CTR mode cipher.
 */
bool cipher_kt_mode_ofb_cfb_ctr (const cipher_kt_t *cipher);
 ...
 */
void cipher_ctx_init (cipher_ctx_t *ctx, uint8_t *key, int key_len, const cipher_kt_t *kt, int enc);
 ...
 *
 * @return 		Cipher mode, either \c OPENVPN_MODE_CBC, \c
 * 			OPENVPN_MODE_OFB or \c OPENVPN_MODE_CFB or \c OPENVPN_MODE_CTR
 */
...
/*
 * Initialises the given HMAC context, using the given digest and key.
 *
