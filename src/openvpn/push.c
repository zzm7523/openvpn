 ...
 *
 * Client received an authentication failed message from server. Runs on client.
 */
void
...
#ifdef ENABLE_MANAGEMENT
		if (management)
			management_notify (management, "info", (const char*) c->sig->signal_text, m);
#endif
	}
}
...
void
incoming_push_message (struct context *c, struct buffer *buffer)
{
	struct gc_arena gc = gc_new ();
	...
	else if (status == PUSH_MSG_REPLY || status == PUSH_MSG_CONTINUATION)
	{
		...
		if (status == PUSH_MSG_REPLY)
		{
			...
			do_up (c, true, c->options.push_option_types_found);
		}
...
send_push_reply (struct context *c)
{
	...
	if (c->c2.push_ifconfig_ipv6_defined)
	{
		...
		buf_printf (&buf, ",ifconfig-ipv6 %s/%d %s", print_in6_addr (c->c2.push_ifconfig_ipv6_local, 0, &gc),
			c->c2.push_ifconfig_ipv6_netbits, print_in6_addr (c->c2.push_ifconfig_ipv6_remote, 0, &gc));
	...
	while (e)
	{
		if (e->enable)
		{
			const size_t len = strlen (e->option);
			if (BLEN (&buf) + len >= safe_cap)
			{
...
#endif
		buf_printf (&buf, ",ifconfig %s %s", print_in_addr_t (ifconfig_local, 0, &gc),
	...
	/* If nothing have been pushed, send an empty push, as the client is expecting a response */
	if (!push_sent)
	{
		bool status = false;

		msg (M_INFO, "send_push_reply(): send an empty push");
...
push_update_digest (md_ctx_t *ctx, struct buffer *buf)
{
	...
	while (buf_parse (buf, ',', line, sizeof (line)))
	{
		...
		{
			md_ctx_update (ctx, (uint8_t *) line, (int) strlen (line));
		}
	}
}

int
process_incoming_push_msg (struct context *c, const struct buffer *buffer, bool honor_received_options,
	unsigned int permission_mask, unsigned int *option_types_found)
{
	...
	if (buf_string_compare_advance (&buf, "PUSH_REQUEST"))
	{
		...
		{
			time_t local_now = openvpn_time (&local_now, MAIN_THREAD_INDEX);
...
#endif
	{
		if (honor_received_options && buf_string_compare_advance (&buf, "PUSH_REPLY"))
		{
			...
			if (ch == ',')
			{
				...
				if (!c->c2.pulled_options_md_init_done)
				{
					const md_kt_t *md5_kt = md_kt_get ("MD5");
					md_ctx_init (c->c2.pulled_options_ctx, md5_kt);
					c->c2.pulled_options_md_init_done = true;
				}
				...
				if (apply_push_options (&c->options, &buf, permission_mask, option_types_found, c->c2.es))
				{
					push_update_digest (c->c2.pulled_options_ctx, &buf_orig);
					switch (c->options.push_continuation)
					{
					case 0:
					case 1:
						md_ctx_final (c->c2.pulled_options_ctx, (uint8_t *) &c->c2.pulled_options_digest);
						md_ctx_cleanup (c->c2.pulled_options_ctx);
						c->c2.pulled_options_md_init_done = false;
						ret = PUSH_MSG_REPLY;
						break;
					case 2:
