...
static const char usage_message[] =
...
#endif
#ifdef ENABLE_GUOMI
	"--lib-path      : Encrypt device lib path.\n"
	"--provider-name : Encrypt device provider name.\n"
	"--test-device   : Check encrypt device status.\n"
	"--device-check-timeout  : Encrypt device check timeout.\n"
#endif
#ifdef ENABLE_MASQUERADE
	"--masq proto salt : Enable masquerade.\n"
#endif
#ifndef WIN32
	"--bind-cpu             : Bind thread to CPU"
#endif
	"--worker-thread        : Worker thread number.\n"
	"--packet-queue-len     : Packet buffer queue length.\n"
	...
	"                  using file.\n"
	"--test-crypto   : Run a self-test of crypto features enabled For debugging only.\n"
...
#ifdef ENABLE_GUOMI
	"--tls-version [GMTLSv1.1|GMTLSv1|TLSv1.2|TLSv1.1|TLSv1|SSLv3] : \n"
	"        sets TLS version we  will accept from the peer.\n"
#else
#if OPENSSL_VERSION_NUMBER < 0x10100000L
	"--tls-version [TLSv1.2|TLSv1.1|TLSv1|SSLv3] : \n"
	"        sets TLS version we  will accept from the peer.\n"
#else
	"--tls-version [TLSv1.3|TLSv1.2|TLSv1.1|TLSv1|SSLv3] : \n"
	"        sets TLS version we  will accept from the peer.\n"
#endif
#endif
...
#ifdef ENABLE_GUOMI
	"--encrypt-cert file     : Local encrypt certificate in .pem format -- must be signed\n"
	"                          by a Certificate Authority in --ca file.\n"
	"--encrypt-key file      : Local encrypt private key in .pem format.\n"
#endif
...
#ifdef ENABLE_GUOMI
#ifndef ENABLE_CRYPTO_POLARSSL
	"--encrypt-pkcs12 file   : PKCS#12 file containing local encrypt private key, local encrypt certificate\n"
	"                          and optionally the root CA certificate.\n"
#endif
#endif
...
#endif
#ifdef ENABLE_GUOMI
	"--gmedapicert select-string : Load the certificate and private key from the\n"
	"                  GUO MI encrypt device.\n"
#endif
	...
	"--hello-window n: TLS hello must start within n seconds.\n"
	...
	"--tls-auth f [d] [n]: Add an additional layer of authentication on top of the TLS\n"
	"                  control channel to protect against DoS attacks.\n"
	"                  f (required) is a shared-secret passphrase file.\n"
	"                  The optional d parameter controls key directionality,\n"
	"                  see --secret option for more info.\n"
	"                  The optional n parameter controls auth alg.\n"
	...
	"                  in an " PACKAGE_NAME " temporary file in [directory]. Peer cert is \n"
...
/*
 * This is where the options defaults go. Any option not explicitly set here will be set to 0.
 */
void
init_options (struct options *o, const bool init_gc)
{
...
#ifdef ENABLE_MASQUERADE
	o->ce.masq_proto = MASQUERADE_PROTO_NONE;	/* 缺省不做链路伪装 */
#endif
	o->verbosity = 1;
	...
	o->sndbuf = 65536;
#ifdef TARGET_LINUX
	o->sockflags = SF_USE_SENDMMSG | SF_USE_RECVMMSG;
	o->tuntap_options.txqueuelen = 1000;
#endif
#ifdef WIN32
#if 0
	...
	o->use_iv = true;
	o->tls_auth_authname = NULL;	/* 必须初始化为NULL */	
	o->tls_auth_authname_defined = false;
...
#ifdef ENABLE_GUOMI
	/* 缺省不定时检查设备 */
	o->device_check_timeout = 0;
	/* 缺省支持所有SSL协议 */
	o->tls_version = "GMTLSv1.1:GMTLSv1:TLSv1.2:TLSv1.1:TLSv1:SSLv3";
#else
	o->tls_version = "TLSv1.2:TLSv1.1:TLSv1:SSLv3";
#endif
	o->key_method = 2;
	o->tls_timeout = 2;
	o->renegotiate_bytes = -1;
	o->renegotiate_seconds = 3600;
	o->hello_window = 10;		/* hello 必须在n秒内启动, 缺省值不能太小 */	
	o->handshake_window = 600;	/* 缺省改为600秒, 使客户端有足够的时间察看、选择证书 */
...
#ifdef ENABLE_GUOMI
	o->worker_thread = 0;		/* 0 根据打开的设备实例数动态决定 */
#else
	o->worker_thread = 2;		/* 缺省启用2个工作线程 */
#endif

	o->packet_queue_len = 1024;

	o->integration = false;
	o->integration_completed = false;
}
...
#ifdef ENABLE_MASQUERADE
	setenv_int_i (es, "masq_proto", e->masq_proto, i);
	setenv_str_i (es, "masq_salt", e->masq_salt, i);
#endif
...
setenv_settings (struct env_set *es, const struct options *o)
{
	...
	setenv_unsigned (es, "daemon_start_time", (unsigned int) time (NULL));
...
get_ipv6_addr_no_netbits (const char *addr, struct gc_arena *gc)
{
	...
	{
		size_t len = end - addr;
		ret = (char *) gc_malloc (len + 1, true, gc);
		memcpy (ret, addr, len);
	}
	return ret;
}
...
{
	msg (D_SHOW_PARMS, "  transport protocol = %s", proto2ascii (o->proto, false));
	SHOW_STR (local);
	SHOW_INT (local_port);
	SHOW_STR (remote);
	...
	msg (D_SHOW_PARMS, "  masquerade protocol = %s", masquerade_proto_2_ascii (o->masq_proto, false));
	SHOW_STR (masq_salt);
...
#ifdef ENABLE_GUOMI
	SHOW_STR (lib_path);
	SHOW_STR (provider_name);
	SHOW_INT (device_check_timeout);
#endif
...
parse_http_proxy_override (const char *server, const char *port, const char *flags,
		const int msglevel, struct gc_arena *gc)
{
	if (server && port)
	{
		...
		ho->http_version = "1.0";
		ho->user_agent = PACKAGE_NAME "-Autoproxy/1.0";
		return ho;
	}
	else
		return NULL;
...
static void
options_postprocess_verify_ce (struct options *options, struct connection_entry *ce)
{
...
#endif
	{
		if (options->tls_client && options->key_method == 2)
			;
		else
	...
	/* If "proto tcp" is specified, make sure we know whether it is tcp-client or tcp-server. */
	...
	if (options->tuntap_options.dhcp_options && options->tuntap_options.ip_win32_type != IPW32_SET_DHCP_MASQ
...
#ifdef ENABLE_FRAGMENT
	if (!proto_is_udp (ce->proto) && ce->fragment > 0)
	...
	if (options->mode == MODE_SERVER)
	{
		...
		{
			const bool ccnr = (options->auth_user_pass_verify_script || PLUGIN_OPTION_LIST (options)
		...
		/* 检查是否支持加密算法 */
		check_cipher_array (options->ciphername, string_array_len (options->ciphername), options->keysize, options->use_iv, true);
		/* 检查是否支持认证算法 */
		check_auth_array (options->authname, string_array_len (options->authname), true);
	}
	else
	{
		/* When not in server mode, err if parameters are specified which require --mode server. */
		if (options->ifconfig_pool_defined || options->ifconfig_pool_persist_filename)
		...
		/* 检查是否支持加密算法 */
		check_cipher_array (options->ciphername, string_array_len (options->ciphername), options->keysize, options->use_iv, false);
		/* 检查是否支持认证算法 */
		check_auth_array (options->authname, string_array_len (options->authname), false);
	}
#endif /* P2MP_SERVER */
	...
	if ((!proto_is_udp (ce->proto)) && (options->replay_window != defaults.replay_window
	...
	/* 启用tls-auth时, 如果没有指定tls_auth_authname, 则tls_auth_authname等同于authname或SHA1 */
	if (options->tls_auth_file && !options->tls_auth_authname_defined)
	{
		if (options->authname_defined)
		{
			if (options->n_authname == 1)
				options->tls_auth_authname = options->authname[0];
			else
				options->tls_auth_authname = "SHA1";
			options->tls_auth_authname_defined = true;
		}
	}
...
#endif
#ifdef ENABLE_GUOMI
			if (options->gmedapi_cert)
				msg (M_USAGE, "Parameter --gmedapicert cannot be used when --pkcs11-provider is also specified.");
#endif
			/* PKCS11 TOKEN 只提供签名证书, 加密证书必须单独提供 */
		}
		else
#endif
#ifdef MANAGMENT_EXTERNAL_KEY
				...
				if (options->cryptoapi_cert)
				{
...
#ifdef ENABLE_GUOMI
					if (options->gmedapi_cert)
						msg (M_USAGE, "Parameter --gmedapicert cannot be used when --cryptoapicert is also specified.");
#endif
#ifdef MANAGMENT_EXTERNAL_KEY
					if (options->management_flags & MF_EXTERNAL_KEY)
					...
					if (options->gmedapi_cert)
					{
						if ((!(options->ca_file)) && (!(options->ca_path)))
							msg (M_USAGE, "You must define CA file (--ca) or CA path (--capath)");
						if (options->cert_file)
							msg (M_USAGE, "Parameter --cert cannot be used when --gmedapicert is also specified.");
						if (options->priv_key_file)
							msg (M_USAGE, "Parameter --key cannot be used when --gmedapicert is also specified.");
						if (options->pkcs12_file)
							msg (M_USAGE, "Parameter --pkcs12 cannot be used when --gmedapicert is also specified.");
#ifdef MANAGMENT_EXTERNAL_KEY
						if (options->management_flags & MF_EXTERNAL_KEY)
							msg (M_USAGE, "Parameter --management-external-key cannot be used when --gmedapicert is also specified.");
#endif
					}
					else
#endif
...
*/
		...
		if (options->gmedapi_cert)
		{
			if (options->encrypt_pkcs12_file)
				msg (M_USAGE, "Parameter --encrypt-pkcs12 cannot be used when --gmedapicert is also specified.");
			if (options->encrypt_cert_file)
				msg (M_USAGE, "Parameter --encrypt-cert cannot be used when --gmedapicert is also specified.");
			if (options->encrypt_priv_key_file)
				msg (M_USAGE, "Parameter --encrypt-key cannot be used when --gmedapicert is also specified.");
		}
		else if (options->encrypt_pkcs12_file)
		{
			if (options->encrypt_cert_file)
				msg (M_USAGE, "Parameter --encrypt-cert cannot be used when --encrypt-pkcs12 is also specified.");
			if (options->encrypt_priv_key_file)
				msg (M_USAGE, "Parameter --encrypt-key cannot be used when --encrypt-pkcs12 is also specified.");
		}
		else
		{
			if (options->encrypt_cert_file != NULL)
			{
#ifdef MANAGMENT_EXTERNAL_KEY
				if ((options->encrypt_priv_key_file != NULL) || (options->management_flags & MF_EXTERNAL_KEY))
#else
				if (options->encrypt_priv_key_file != NULL)
#endif
					;
				else
					msg (M_USAGE, "If you use one of --encrypt-cert or --encrypt-key, you must use them both");
			}
			else
			{
				if (options->encrypt_priv_key_file != NULL)
		...
		/* 如果加密证书未指定, 设置加密证书等同于签名证书 */
		if (!options->encrypt_cert_file)
			options->encrypt_cert_file = options->cert_file;
		if (!options->encrypt_cert_file_inline)
			options->encrypt_cert_file_inline = options->cert_file_inline;

		if (!options->encrypt_priv_key_file)
			options->encrypt_priv_key_file = options->priv_key_file;
		if (!options->encrypt_priv_key_file_inline)
			options->encrypt_priv_key_file_inline = options->priv_key_file_inline;

		if (!options->encrypt_pkcs12_file)
			options->encrypt_pkcs12_file = options->pkcs12_file;
		if (!options->encrypt_pkcs12_file_inline)
			options->encrypt_pkcs12_file_inline = options->pkcs12_file_inline;
#endif
	}
	else
	{
		/* Make sure user doesn't specify any TLS options when in non-TLS mode. */
...
#endif
#ifdef ENABLE_GUOMI
		MUST_BE_UNDEF (encrypt_cert_file);
		MUST_BE_UNDEF (encrypt_priv_key_file);
#ifndef ENABLE_CRYPTO_POLARSSL
		MUST_BE_UNDEF (encrypt_pkcs12_file);
#endif
#endif
...
options_postprocess_mutate_ce (struct options *o, struct connection_entry *ce)
{
	...
	/* Set MTU defaults */
	{
		...
		if ((dev == DEV_TYPE_TAP) && !ce->tun_mtu_extra_defined)
		{
			...
			ce->tun_mtu_extra = -1;	/* -1表示缺省值, 大于等于0表示用户明确指定的值 */
		}
	}
}
...
options_postprocess_mutate_invariant (struct options *options)
{
	...
	by default if the user asks for PKCS#11 without otherwise specifying the module to use. */
...
static void
options_postprocess_verify (struct options *o)
{
#ifdef WIN32
	if (o->verbosity < 3)
	{
		/* Windows GUI 客户端交互要求 verb >= 3 */
		o->verbosity = 3;
		msg (M_WARN, "Windows platform --verb must greater than or equal 3");
	}
#endif
...
{
	/*
	 * Process helper-type options which map to other, more complex sequences of options.
	 */
...
check_file_access (const int type, const char *file, const int mode, const char *opt)
{
	...
	if (errcode > 0)
		msg (M_NOPREFIX|M_OPTERR, "%s fails with '%s': %s", opt, file, strerror (errno));
...
check_cmd_access (const char *command, const char *opt, const char *chroot)
{
	...
	if (argv.arg_v[0])
		 ...
		 */
		return_code = check_file_access_chroot (chroot, CHKACC_FILE, argv.arg_v[0], X_OK, opt);
	else
	{
...
/*
 * Sanity check of all file/dir options.  Checks that file/dir is accessible by OpenVPN
 */
static void
options_postprocess_filechecks (struct options *options)
{
	...
	/* ** Password files ** */
#ifdef ENABLE_SSL
	errs |= check_file_access (CHKACC_FILE|CHKACC_ACPTSTDIN, options->key_pass_file, R_OK, "--askpass");
#endif /* ENABLE_SSL */
#ifdef ENABLE_MANAGEMENT
	errs |= check_file_access (CHKACC_FILE|CHKACC_ACPTSTDIN, options->management_user_pass, R_OK,
...
#endif /* ENABLE_SSL */
#if P2MP_SERVER
	errs |= check_file_access_chroot (options->chroot_dir, CHKACC_FILE, options->client_config_dir, R_OK|X_OK,
		"--client-config-dir");
	errs |= check_file_access_chroot (options->chroot_dir, CHKACC_FILE, options->tmp_dir, R_OK|W_OK|X_OK,
 ...
 * Also set some options based on other options.
 */
void
...
{
	/* we will stop sending 'proto xxx' in OCC in a future version (because it's not useful),
	 * and to reduce questions when interoperating, we start not-printing a warning about it today
	 */
				...
				if (!strcmp (p1_prefix, p2_prefix))
				{
					if (report_inconsistent)
						msg (msglevel, "WARNING: '%s' is used inconsistently, %s='%s', %s='%s'",
							safe_print (p1_prefix, &gc), b1_name, safe_print (p1, &gc), b2_name, safe_print (p2, &gc));
					goto done;
				}
			}
		}
...
read_inline_file (struct in_src *is, const char *close_tag, struct gc_arena *gc)
{
	...
	{
		char *line_ptr = line;
		while (isspace (*line_ptr)) line_ptr++;	/* Remove leading spaces */
		...
		if (!buf_safe (&buf, (int) strlen (line) + 1))
		{
...
static void
add_option (struct options *options, char *p[], const char *file, int line, const int level, const int msglevel,
		const unsigned int permission_mask, unsigned int *option_types_found, struct env_set *es);

static void
read_config_file (struct options *options, const char *file, int level, const char *top_file, const int top_line,
...
static void
read_config_string (const char *prefix, struct options *options, const char *config, const int msglevel,
		const unsigned int permission_mask, unsigned int *option_types_found, struct env_set *es)
{
	char line[OPTION_LINE_SIZE];
	struct buffer multiline;
	...
	buf_set_read (&multiline, (uint8_t *) config, (int) strlen (config));
...
bool
apply_push_options (struct options *options, struct buffer *buf, unsigned int permission_mask,
...
void
options_server_import (struct options *o, const char *filename, int msglevel, unsigned int permission_mask,
		unsigned int *option_types_found, struct env_set *es)
{
	msg (D_PUSH, "OPTIONS IMPORT: reading client specific options from: %s", filename);
	read_config_file (o, filename, 0, filename, 0, msglevel, permission_mask, option_types_found, es);
}
...
static bool
verify_permission (const char *name, const char *file, int line, const unsigned int type, const unsigned int allowed,
		unsigned int *found, const int msglevel, struct options* options)
{
	if (!(type & allowed))
	{
...
#ifndef ENABLE_SMALL
	/*
	 * Check if this options is allowed in connection block, but we are currently not in a connection
	 * block Parsing a connection block uses a temporary options struct without connection_list
	 */
...
/*
 * Check that an option doesn't have too many parameters.
 */
...
static void
set_user_script (struct options *options, const char **script, const char *new_script, const char *type,
		bool in_chroot)
{
	if (*script)
	{
		msg (M_WARN, "Multiple --%s scripts defined.  The previously configured script is overridden.", type);
	}
	*script = new_script;
		...
		const unsigned int permission_mask, unsigned int *option_types_found, struct env_set *es)
{
	...
	/* If directive begins with "setenv opt" prefix, don't raise an error if directive is unrecognized. */
	...
	else if (streq (p[0], "echo") || streq (p[0], "parameter"))
	{
		...
		{
#if 0
			/* removed for now since ECHO can potentially include security-sensitive strings */
	...
	else if (streq (p[0], "ipchange") && p[1])
	{
		...
		if (!no_more_than_n_args (msglevel, p, 2, NM_QUOTE_HINT))
			goto err;
		set_user_script (options, &options->ipchange, string_substitute (p[1], ',', ' ', &options->gc),
			"ipchange", true);
	...
	else if (streq (p[0], "ip-win32") && p[1])
	{
		...
		if (index < 0)
		{
			msg (msglevel, "Bad --ip-win32 method: '%s'.  Allowed methods: %s", p[1], ipset2ascii_all (&gc));
			goto err;
		}
		...
		if (index == IPW32_SET_DHCP_MASQ)
		{
			if (p[2])
			{
				...
				if (p[3])
				{
					...
					{
						msg (msglevel, "--ip-win32 dynamic [offset] [lease-time]: lease time parameter (%d) must be at least %d seconds",
	...
	else if (streq (p[0], "block-outside-dns") && !p[1])
	{
		...
		{
			msg (msglevel_fc, "Failed to enable --block-outside-dns. Maybe WFP is not supported on your system?");
			goto err;
		}
	}
#endif
	...
	else if (streq (p[0], "tls-auth-algo") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->tls_auth_authname_defined = true;
		options->tls_auth_authname = p[1];
		if (streq (options->tls_auth_authname, "none"))
		{
			options->tls_auth_authname_defined = false;
			options->tls_auth_authname = NULL;
		}
	}
	else if (streq (p[0], "key-direction") && p[1])
	{
		...
		VERIFY_PERMISSION (OPT_P_GENERAL);
		key_direction = ascii2keydirection (msglevel, true, p[1]);
		if (key_direction >= 0)
	...
	else if (streq (p[0], "secret") && p[1])
	{
		...
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			...
			if (p[2])
			{
				...
				key_direction = ascii2keydirection (msglevel, true, p[2]);
				if (key_direction >= 0)
					options->key_direction = key_direction;
				else
					goto err;
	...
	else if (streq (p[0], "auth") && p[1])
	{
		...
		if (streq (p[1], "none"))
		{
			...
			string_array_reset ((char**) options->authname, AUTH_LIST_SIZE, false);
		}
		else
		{
			options->authname_defined = true;
			if (!string_array_has (options->authname, p[1]))
			{
				if (options->n_authname >= AUTH_LIST_SIZE)
					msg (msglevel, "Maximum number of 'auth' options (%d) exceeded", AUTH_LIST_SIZE);
				else
					options->authname[options->n_authname] = p[1];
			}
		}
		options->n_authname = string_array_len (options->authname);
	}
	else if (streq (p[0], "auth"))
	{
		...
		options->authname_defined = true;
		if (string_array_len (options->authname) == 0)
			options->authname[0] = "SHA1";
		options->n_authname = string_array_len (options->authname);
	}
	else if (streq (p[0], "cipher") && p[1])
	{
		...
		if (streq (p[1], "none"))
		{
			...
			string_array_reset ((char**) options->ciphername, CIPHER_LIST_SIZE, false);
		}
		else
		{
			options->ciphername_defined = true;
			if (!string_array_has (options->ciphername, p[1]))
			{
				if (options->n_ciphername >= CIPHER_LIST_SIZE)
					msg (msglevel, "Maximum number of 'cipher' options (%d) exceeded", CIPHER_LIST_SIZE);
				else
					options->ciphername[options->n_ciphername] = p[1];
			}
		}
		options->n_ciphername = string_array_len (options->ciphername);
	}
	else if (streq (p[0], "cipher"))
	{
		...
		options->ciphername_defined = true;
		if (string_array_len (options->ciphername) == 0)
			options->ciphername[0] = "BF-CBC";
		options->n_ciphername = string_array_len (options->ciphername);
	}
...
#endif
#ifdef ENABLE_GUOMI
	else if (streq (p[0], "gmedapicert") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->gmedapi_cert = p[1];
	}
#endif
...
#ifdef ENABLE_GUOMI
	else if (streq (p[0], "encrypt-cert") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->encrypt_cert_file = p[1];
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			options->encrypt_cert_file_inline = p[2];
		}
	}
	else if (streq (p[0], "encrypt-key") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->encrypt_priv_key_file = p[1];
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			options->encrypt_priv_key_file_inline = p[2];
		}
	}
#ifndef ENABLE_CRYPTO_POLARSSL
	else if (streq (p[0], "encrypt-pkcs12") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->encrypt_pkcs12_file = p[1];
		if (streq (p[1], INLINE_FILE_TAG) && p[2])
		{
			options->encrypt_pkcs12_file_inline = p[2];
		}
	}
#endif
#endif
	else if (streq (p[0], "tls-version") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		if (!tls_version_check (p[1]))
		{
			msg (msglevel, "unknown tls-version parameter: %s", p[1]);
			goto err;
		}
		else
			options->tls_version = p[1];
	}
#ifndef ENABLE_CRYPTO_POLARSSL
	...
	else if (streq (p[0], "verify-x509-name") && p[1] && strlen (p[1]))
	{
		...
		{
			msg (msglevel, "you cannot use --verify-x509-name with --compat-names or --no-name-remapping");
			goto err;
		}
		if (p[2])
		{
	...
	else if (streq (p[0], "hello-window") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_TLS_PARMS);
		options->hello_window = positive_atoi (p[1]);
	}
	...
	else if (streq (p[0], "tls-auth") && p[1])
	{
		...
		{
			int key_direction = ascii2keydirection (msglevel, false, p[2]);
			if (key_direction >= 0)
				options->key_direction = key_direction;
			else
			{
	...
	else if (streq (p[0], "key-method") && p[1])
	{
		...
		if (key_method < KEY_METHOD_MIN || key_method > KEY_METHOD_MAX)
		{
			msg (msglevel, "key_method parameter (%d) must be >= %d and <= %d",
				key_method, KEY_METHOD_MIN, KEY_METHOD_MAX);
			goto err;
		}
	...
	else if (streq (p[0], "x509-username-field") && p[1])
	{
		...
		if (strncmp ("ext:", s, 4) != 0)
		{
			...
			if (strlen (s) == i)
			{
				...
				msg (M_WARN, "DEPRECATED FEATURE: automatically upcased the --x509-username-field "
					"parameter to '%s'; please update your configuration", p[1]);
			}
		}
...
#ifdef ENABLE_GUOMI
	else if (streq (p[0], "lib-path") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->lib_path = p[1];
	}
	else if (streq (p[0], "provider-name") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->provider_name = p[1];
	}
	else if (streq (p[0], "device-check-timeout") && p[1])
	{
        int timeout = atoi (p[1]);

		VERIFY_PERMISSION (OPT_P_GENERAL);
        if (timeout < MIN_DEVICE_CHECK_TIMEOUT || timeout > MAX_DEVICE_CHECK_TIMEOUT)
		{
            msg (msglevel, "device-check-timeout parameter (%d) must be >= %d and <= %d",
                    timeout,
                    MIN_DEVICE_CHECK_TIMEOUT,
                    MAX_DEVICE_CHECK_TIMEOUT);
            goto err;
        }
		else
	        options->device_check_timeout = timeout;
	}
	else if (streq (p[0], "test-device"))
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
...
#endif
#ifdef ENABLE_MASQUERADE
	else if (streq (p[0], "masq") && p[1] && p[2])
	{
		int masq_proto;

		VERIFY_PERMISSION (OPT_P_GENERAL|OPT_P_CONNECTION);
		masq_proto = ascii_2_masquerade_proto (p[1]);
		if (masq_proto < 0)
		{
			msg (msglevel, "Bad masquerade protocol: '%s'. Allowed protocols with --masq option: %s",
				p[1],
				masquerade_proto_2_ascii_all (&gc));
			goto err;
		}
		else
			options->ce.masq_proto = masq_proto;

		if (strlen (p[2]) < 8 || strlen (p[2]) > 256)
		{
			msg (msglevel, "masquerade salt length must be >= %d and <= %d", 8, 256);
			goto err;
		}
		else
			options->ce.masq_salt = p[2];
	}
#endif
#ifdef TARGET_LINUX
	else if (streq (p[0], "bind-cpu"))
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->bind_cpu = true;
	}
#endif
	else if (streq (p[0], "worker-thread") && p[1])
	{
		int worker_thread = atoi (p[1]);

		VERIFY_PERMISSION (OPT_P_GENERAL);
		if (worker_thread == 0)
		{
#ifdef ENABLE_GUOMI
			/* worker_thread == 0 表示自动计算工作线程数 */
			options->worker_thread = 0;
#else
			msg (msglevel, "worker-thread parameter (%d) must be >= %d and <= %d",
				worker_thread, MIN_WORKER_THREAD, MAX_WORKER_THREAD);
			goto err;
#endif
		}
		else
		{
			if (worker_thread < MIN_WORKER_THREAD || worker_thread > MAX_WORKER_THREAD)
			{
				msg (msglevel, "worker-thread parameter (%d) must be >= %d and <= %d",
					worker_thread, MIN_WORKER_THREAD, MAX_WORKER_THREAD);
				goto err;
				...
				options->worker_thread = worker_thread;
		}
	}
	else if (streq (p[0], "packet-queue-len") && p[1])
	{
		int packet_queue_len = atoi (p[1]);

		VERIFY_PERMISSION (OPT_P_GENERAL);
		if (packet_queue_len < MIN_PACKET_QUEUE_LEN || packet_queue_len > MAX_PACKET_QUEUE_LEN)
		{
			msg (msglevel, "packet-queue-len parameter (%d) must be >= %d and <= %d",
				packet_queue_len, MIN_PACKET_QUEUE_LEN, MAX_PACKET_QUEUE_LEN);
			goto err;
		}
		else
			options->packet_queue_len = packet_queue_len;
	}
#ifdef WIN32
	else if (streq (p[0], "ctl-processid") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->ctl_processid = (DWORD) _atoi64 (p[1]);
	}
#endif
	else if (streq (p[0], "integration") && !p[1])
	{
		VERIFY_PERMISSION (OPT_P_GENERAL);
		options->integration = true;
	}
	else if (streq (p[0], "policy") && p[1])
	{
		VERIFY_PERMISSION (OPT_P_PROPRIETARY);
		if (!options->integration_completed)
		{
			int i;
			bool good = true;
			struct buffer string = alloc_buf_gc (OPTION_PARM_SIZE, &gc);
			...
			if (!options->policys)
				options->policys = buffer_list_new (MAX_POLICY_SIZE);
			for (i = 1; i < MAX_PARMS; ++i)
			{
				...
				if (i > 1)
					good &= buf_printf (&string, " ");
				good &= buf_printf (&string, "\"%s\"", p[i]);
			}
			if (good)
			{
				if (options->policys->size >= options->policys->max_size)
					msg (M_FATAL, PACKAGE_NAME "POLICY: cannot add more than %d policy", options->policys->max_size);
				else
				{
					msg (M_INFO, "POLICY: %s", BSTR (&string));
					buffer_list_push (options->policys, BPTR (&string));
				}
			}
			else
				msg (M_WARN, "policy option overflow");
		}
	}
	else
	{
		...
		/* Check if an option is in --ignore-unknown-option and set warning level to non fatal */
		for (i=0; options->ignore_unknown_option && options->ignore_unknown_option[i]; i++)
		{
