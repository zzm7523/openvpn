...
ifconfig_pool_entry_free (struct ifconfig_pool_entry *ipe, bool hard)
{
	...
	if (hard)
		...
		ipe->last_release = now_sec (MAIN_THREAD_INDEX);
}
...
ifconfig_pool_find (struct ifconfig_pool *pool, const char *common_name)
{
	...
	for (i = 0; i < pool->size; ++i)
	{
		...
		if (!ipe->in_use)
		{
			...
			/* Keep track of the unused IP address entry which was released earliest. */
			...
			/* Keep track of a possible allocation to us from an earlier session. */
		...
		const bool ipv6_pool, const struct in6_addr ipv6_base, const int ipv6_netbits)
{
	...
	if (pool->ipv6)
	{
		...
		/* the current code is very simple and assumes that the IPv6 pool is at least
		 * as big as the IPv4 pool, and we don't need to do separate math etc. for IPv6
		 */
	...
	msg (D_IFCONFIG_POOL, "IFCONFIG POOL: base=%s size=%d, ipv6=%d",
		print_in_addr_t (pool->base, 0, &gc), pool->size, pool->ipv6);
...
ifconfig_pool_set (struct ifconfig_pool* pool, const char *cn, const in_addr_t addr, const bool fixed)
{
	...
	if (h >= 0)
	{
		...
		e->last_release = now_sec (MAIN_THREAD_INDEX);
		e->fixed = fixed;
	}
}
			...
			if (e->common_name)
			{
				...
				if (pool->ipv6)
				{
					...
					status_printf (out, "%s,%s,%s",
						e->common_name, print_in_addr_t (ip, 0, &gc), print_in6_addr (ip6, 0, &gc));
				}
				else
				{
	...
	if (persist && persist->file && pool)
	{
		...
		while (true)
		{
			...
			if (BLEN (&in))
			{
				...
				if (c == '#' || c == ';')
					continue;
				msg (M_INFO, "ifconfig_pool_read(), in='%s', TODO: IPv6", BSTR (&in));
