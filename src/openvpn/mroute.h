...
/*
 * Used to help maintain CIDR routing table. 只有主线程能修改
 */
struct mroute_helper
{
	int ageable_ttl_secs;          /* host route cache entry time-to-live, 不能更改 */
	volatile unsigned int cache_generation; /* incremented when route added */
	volatile int n_net_len;                 /* length of net_len array */
	volatile uint8_t net_len[MR_HELPER_NET_LEN];      /* CIDR netlengths in descending order */
	volatile int net_len_refcount[MR_HELPER_NET_LEN]; /* refcount of each netlength */
};
...
/*
 * Given a raw packet in buf, return the src and dest addresses of the packet.
 */
static inline unsigned int
mroute_extract_addr_from_packet (struct mroute_addr *src, struct mroute_addr *dest, struct mroute_addr *esrc,
		struct mroute_addr *edest, const struct buffer *buf, int tunnel_type)
{
	unsigned int mroute_extract_addr_ipv4 (struct mroute_addr *src, struct mroute_addr *dest, const struct buffer *buf);
	unsigned int mroute_extract_addr_ether (struct mroute_addr *src, struct mroute_addr *dest,
			struct mroute_addr *esrc, struct mroute_addr *edest, const struct buffer *buf);
