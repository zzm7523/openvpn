...
reliable_ack_read (struct reliable_ack *ack, struct buffer *buf, const struct session_id *sid)
{
	...
	if (count)
	{
		if (!session_id_read (&session_id_remote, buf))
			goto error;
		if (!session_id_defined (&session_id_remote) || !session_id_equal (&session_id_remote, sid))
		{
...
bool
reliable_ack_write (struct reliable_ack *ack, struct buffer *buf, const struct session_id *sid,
		int max, bool prepend)
{
	int i, j;
	uint8_t n;
	struct buffer sub;
...
reliable_can_send (const struct reliable *rel)
{
	...
	for (i = 0; i < rel->size; ++i)
	{
		...
		{
			++n_active;
			if (now_sec (MAIN_THREAD_INDEX) >= e->next_try)
				++n_current;
		}
	}
...
reliable_schedule_now (struct reliable *rel)
{
	...
	for (i = 0; i < rel->size; ++i)
	{
		...
		if (e->active)
		{
			e->next_try = now_sec (MAIN_THREAD_INDEX);
...
void
reliable_mark_active_incoming (struct reliable *rel, struct buffer *buf, packet_id_type pid, int opcode)
{
	int i;
	for (i = 0; i < rel->size; ++i)
	{
...
reliable_mark_active_outgoing (struct reliable *rel, struct buffer *buf, int opcode)
{
	...
	for (i = 0; i < rel->size; ++i)
	{
		...
		if (buf == &e->buf)
		{
			/* Write mode, increment packet_id (i.e. sequence number) linearly and prepend id to packet */
			packet_id_type net_pid;
