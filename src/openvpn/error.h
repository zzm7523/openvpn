 ...
 * If OPENVPN_DEBUG_COMMAND_LINE is defined, contents of argc/argv will
 * be dumped to OPENVPN_DEBUG_FILE as well as all other OpenVPN messages.
 */
...
/* Function prototypes */
void init_msg_mutex (void);

void uninit_msg_mutex (void);
...
#define ASSERT_FATAL(x) do { if (!(x)) exit (1); } while (false)
...
/* In multiclient mode, put a client-specific prefix before each message. */
struct msg_prefix
{
	const char *prefix;
...
extern struct msg_prefix global_msg_prefixs[MAX_THREAD_INDEX];

static inline void
msg_set_prefix (const char *prefix, const int thread_idx)
{
#ifdef THREAD_ACCESS_CHECK
	ASSERT (THREAD_SELF_INDEX() == thread_idx && (sizeof (global_msg_prefixs[0]) & (CACHE_LINE_SIZE - 1)) == 0);
#endif
	global_msg_prefixs[thread_idx].prefix = prefix;
}

/* 只有在verb较高时, 才可能被调用; 正常情况下应该不会被调用 */
static inline const char *
msg_get_prefix (void)
{
	const int thread_idx = get_thread_index (pthread_self ());
	ASSERT (thread_idx >= 0 && thread_idx < MAX_THREAD_INDEX);
	return global_msg_prefixs[thread_idx].prefix;
}
...
/* Return true if this is a system error which can be safely ignored. */
static inline bool
...
#if 1 /* if enabled, suppress ENOBUFS errors */
#ifdef ENOBUFS
