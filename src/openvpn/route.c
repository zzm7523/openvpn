...
#ifdef WIN32
#include "winsock2.h"
#define  WAIT_SERVICE_TIMEOUT 5000
...
{
	const size_t rl_size = 
		array_mult_safe (sizeof (struct route_option), src->capacity, sizeof (struct route_option_list));
...
{
	const size_t rl_size =
		array_mult_safe (sizeof (struct route_ipv6_option), src->capacity, sizeof (struct route_ipv6_option_list));
...
copy_route_option_list (struct route_option_list *dest, const struct route_option_list *src)
{
	...
	if (src->capacity > dest->capacity)
		msg (M_FATAL,
			PACKAGE_NAME " ROUTE: (copy) number of route options in src (%d) is greater than route list capacity in dest (%d)",
...
fail:
	msg (M_WARN, PACKAGE_NAME " ROUTE: failed to parse/resolve route for host/network: %s", ro->network);
	return false;
}
...
fail:
	msg (M_WARN, PACKAGE_NAME " ROUTE: failed to parse/resolve route for host/network: %s", r6o->prefix);
	r6->defined = false;
	return false;
}
...
void
add_route_ipv6_to_option_list (struct route_ipv6_option_list *l, const char *prefix, const char *gateway,
		const char *metric)
{
...
static void
add_block_local_item (struct route_list *rl, const struct route_gateway_address *gateway, in_addr_t target)
{
	const int rgi_needed = (RGI_ADDR_DEFINED|RGI_NETMASK_DEFINED);
	if ((rl->rgi.flags & rgi_needed) == rgi_needed && rl->rgi.gateway.netmask < 0xFFFFFFFF
			&& (rl->n)+2 <= rl->capacity)
	{
		struct route_ipv4 r;
		unsigned int l2;
...
{
	const int rgi_needed = (RGI_ADDR_DEFINED|RGI_NETMASK_DEFINED);
	if ((rl->flags & RG_BLOCK_LOCAL) && (rl->rgi.flags & rgi_needed) == rgi_needed
			&& (rl->spec.flags & RTSA_REMOTE_ENDPOINT)
			&& rl->spec.remote_host_local != TLA_LOCAL)
	{
		...
		for (i = 0; i < (size_t) rl->rgi.n_addrs; ++i)
		{
...
init_route_list (struct route_list *rl, const struct route_option_list *opt,
		const char *remote_endpoint, int default_metric, in_addr_t remote_host, struct env_set *es)
{
	struct gc_arena gc = gc_new ();
	bool ret = true;
...
init_route_ipv6_list (struct route_ipv6_list *rl6, const struct route_ipv6_option_list *opt6,
		const char *remote_endpoint, int default_metric, struct env_set *es)
{
	struct gc_arena gc = gc_new ();
	bool ret = true;
	...
	/* "default_gateway" is stuff for "redirect-gateway", which we don't do for IPv6 yet -> TODO */
	{
		...
		unsigned int flags, const struct route_gateway_info *rgi, const struct env_set *es)
{
	...
	{
		if (rb->bypass[i])
			add_route3 (rb->bypass[i], IPV4_NETMASK_HOST, gateway, tt, flags | ROUTE_REF_GW, rgi, es);
	}
}
	...
	if (rl && rl->flags & RG_ENABLE)
	{
		...
		else
		{
			...
			if (!local)
			{
				...
				/* if remote_host is not ipv4 (ie: ipv6), just skip adding this special /32 route */
		...
		const struct route_gateway_info *rgi)
{
		...
		&& netmask == 0xFFFFFFFF)
	{
		...
		else
		{
			...
			size_t i;
			for (i = 0; i < (size_t) rgi->n_addrs; ++i)
			{
...
void
add_route (struct route_ipv4 *r, const struct tuntap *tt, unsigned int flags, const struct route_gateway_info *rgi, /* may be NULL */
		const struct env_set *es)
{
	...
	argv_printf (&argv, "%s route add %s/%d", iproute_path, network, count_netmask_bits (netmask));
...
#else
	argv_printf (&argv, "%s add -net %s netmask %s", ROUTE_PATH, network, netmask);
	if (r->flags & RT_METRIC_DEFINED)
		argv_printf_cat (&argv, "metric %d", r->metric);
	if (is_on_link (is_local_route, flags, rgi))
		argv_printf_cat (&argv, "dev %s", rgi->iface);
...
static const char * 
print_in6_addr_netbits_only (struct in6_addr network_copy, int netbits, struct gc_arena * gc)
{
...
add_route_ipv6 (struct route_ipv6 *r6, const struct tuntap *tt, unsigned int flags, const struct env_set *es)
{
...
#ifdef WIN32
	index = get_adapter_index_flexible (device);
	...
	/* Filter out routes which are essentially no-ops (not currently done for IPv6) */
	 ...
	 */
	if (tt->type == DEV_TYPE_TAP && !(r6->metric_defined && r6->metric == 0))
	{
		gateway_needed = true;
	}
	...
	argv_printf (&argv, "%s%sc interface ipv6 add route %s/%d %s",
		...
		network,
		r6->netbits,
		win32_version_info () == WIN_XP ? device : iface);
...
static void
delete_route (struct route_ipv4 *r, const struct tuntap *tt, unsigned int flags, const struct route_gateway_info *rgi,
		const struct env_set *es)
{
	struct gc_arena gc = gc_new ();
	struct argv argv;
...
/*
 * The --redirect-gateway option requires OS-specific code below to get the current default gateway.
 */
...
static void
test_route_helper (bool *ret, int *count, int *good, int *ambig, const IP_ADAPTER_INFO *adapters, const in_addr_t gateway)
{
	int c;
...
show_routes (int msglev)
{
	...
	if (rt)
	{
		for (i = 0; i < (int) rt->dwNumEntries; ++i)
		{
...
get_default_gateway (struct route_gateway_info *rgi)
{
	...
	/* Android, set some pseudo GW, addr is in host byte order, Determining the default GW
	 * on Android 5.0+ is non trivial and serves almost no purpose since OpenVPN only uses
	 * the default GW address to add routes for networks that should NOT be routed over the
	 * VPN. Using a well known address (127.'d'.'g'.'w') for the default GW make detecting
