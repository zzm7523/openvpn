...
#define MAX_PRI_KEY_PASS_INPUT_COUNT 3
...
struct key_state
{
	...
	/* key_ctx_bi数组, 每个线程对应一个key_ctx_bi */
	struct key_ctx_bi key[MAX_THREAD_INDEX]; /* data channel keys for encrypt/decrypt/hmac */
	...
	/* key_stats数组, 每个线程一个key_stats对象 */
	struct key_stats ks_stats[MAX_THREAD_INDEX];
...
struct tls_options
{
...
#ifdef ENABLE_OCC
	/* local and remote options strings that must match between client and server */
	...
	int hello_window;	/* 接受服务端hello的时间窗口, UDP用来确认网关可达 */
	...
	/* allow openvpn config info to be passed over control channel */
 ...
 *
 *  @{ */
#define KS_SIZE       3         /**< Size of the \c tls_session.key array. */
/** @} name Index of key_state objects within a tls_session structure */
/** @} addtogroup control_processor */
...
struct tls_session
{
	...
	struct key_state *ks_primary;
	struct key_state *ks_lame_duck;
	struct key_state *ks_garbage;
...
struct tls_multi
{
	...
	/**< List of \c key_state objects in the order they should be scanned by data channel modules. */
	...
	int64_t local_key_id;	/* increments with each hard or soft reset (for key renegotiation) */

	/* Our session objects. */
	struct tls_session session[TM_SIZE];
	/**< Array of \c tls_session objects representing control channel sessions with the remote peer. */
