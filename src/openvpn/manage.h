...
#endif
#ifdef MANAGEMENT_PF
	bool (*client_pf) (void *arg, const unsigned long cid, struct buffer_list *pf_config);
#endif
...
/*
 * The management object keeps track of OpenVPN --echo parameters.
 */
...
man_bytecount_possible_output_client (struct management *man)
{
	if (man->connection.bytecount_update_seconds > 0
		&& now_sec (MAIN_THREAD_INDEX) >= man->connection.bytecount_last_update + man->connection.bytecount_update_seconds)
	{
		...
		struct man_def_auth_context *mdac)
{
	...
	if (man->connection.bytecount_update_seconds > 0
		&& now_sec (MAIN_THREAD_INDEX) >= mdac->bytecount_last_update + man->connection.bytecount_update_seconds
