...
/*
 * Process server, server-bridge, and client helper directives after the parameters themselves
 * have been parsed and placed in struct options.
 */
void
helper_client_server (struct options *o)
{
	...
	if (o->server_ipv6_defined)
	{
		...
		o->ifconfig_ipv6_pool_base = add_in6_addr (o->server_network_ipv6, 0x1000);
