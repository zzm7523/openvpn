...
#define N_DHCP_ADDR 4        /* Max # of addresses allowed for DNS, WINS, etc. */
...
# define TUNNEL_TOPOLOGY(tt) ((tt) ? ((tt)->topology) : TOP_UNDEF)
	...
	bool enable_dhcp;		/* 关闭tun设备时, 重新启用dhcp */
	...
	/* used for setting interface address via IP Helper API or DHCP masquerade */
	bool ipapi_context_defined;
	ULONG ipapi_context;
	ULONG ipapi_instance;
	in_addr_t adapter_netmask;

	/* Windows adapter index for TAP-Windows adapter, ~0 if undefined */
	DWORD adapter_index;
...
void do_ifconfig (struct tuntap *tt, const char *actual,	/* actual device name */
...
/*
 * Should ifconfig be called before or after tun dev open?
 */
...
DWORD get_adapter_index_flexible (const char *name);
...
int tun_read_queue (struct tuntap *tt, int maxsize, struct buffer *buf);
...
{
	/* This corresponds to the STATUS_NO_SUCH_DEVICE error in tapdrvr.c. */
	if (status < 0)
	{
...
tun_write_win32 (struct tuntap *tt, struct buffer *buf)
{
	...
	if (overlapped_io_active (&tt->writes))
	{
		...
		if (tt->writes.iostate == IOSTATE_QUEUED) /* 包还不能放入发送缓存 */
			return 0;
		else
		{
			ASSERT (tt->writes.iostate == IOSTATE_INITIAL);
...
read_tun_buffered (struct tuntap *tt, struct buffer *buf, int maxsize)
{
	...
	{
		status = tun_finalize (tt->hand, &tt->reads, buf);
		if (status < 0)
			err = GetLastError();
	}
	else
	{
		tun_read_queue (tt, 0, buf);
	}
	...
	{
		return tt->reads.iostate == IOSTATE_QUEUED ? 0 : BLEN (buf);
	}
}
