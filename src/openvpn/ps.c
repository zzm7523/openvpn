...
/*
 * A foreign (non-OpenVPN) connection we are proxying, usually HTTPS
 */
...
/*
 * Close most of parent's fds. Keep stdin/stdout/stderr, plus one
 * other fd which is presumed to be our pipe back to parent.
 * Admittedly, a bit of a kludge, but posix doesn't give us a kind
 * of FD_CLOEXEC which will stop fds from crossing a fork().
 */
static void
...
/*
 * Usually we ignore signals, because our parent will deal with them.
 */
static void
set_signals (void)
{
 ...
 */
static void
port_share_sendmsg (const socket_descriptor_t sd, const char command,
		const struct buffer *head, const socket_descriptor_t sd_send)
{
	if (socket_defined (sd))
	{
		struct msghdr mesg;
...
/*
 * Run through the proxy entry list and delete all entries marked for close.
 */
static void
...
static void
sock_addr_set (struct openvpn_sockaddr *osaddr, const in_addr_t addr, const int port)
{
	CLEAR (*osaddr);
...
/*
 * Create a new pair of proxy_connection entries, one for each socket file descriptor
 * involved in the proxy. We are given the client fd, and we should derive our own
 * server fd by connecting to the server given by server_addr/server_port. 
 * Return true on success and false on failure to connect to server.
 */
static bool
...
/*
 * This function runs in the context of the background proxy process. Receive a control
 * message from the parent (sent by the port_share_sendmsg function above) and act on it.
 * Return false if the proxy process should exit, true otherwise.
 */
static bool
	...
	*((socket_descriptor_t*) CMSG_DATA (h)) = SOCKET_UNDEFINED;
	...
	if (status != -1)
	{
		...
		else
		{
			...
			{
				buf.len = status - 1;
				if (proxy_entry_new (list, es, server_addr, server_port, received_fd,
						&buf, journal_dir))
				{
...
proxy_connection_io_send (struct proxy_connection *pc, int *bytes_sent)
{
	...
	else
	{
		*bytes_sent += status;
		...
		{
			dmsg (D_PS_PROXY_DEBUG, "PORT SHARE PROXY: partial write[%d], tried=%d got=%d",
				(int) sd, pc->buf.len, status);
		...
		const int max_initial_buf, const char *journal_dir)
{
	if (send_control (sd_control, RESPONSE_INIT_SUCCEEDED) >= 0)
	{
		...
		while (true)
		{
			...
			n_events = event_wait (es, &tv, esr, SIZE (esr), &wakeup);
 ...
 */
struct port_share *
port_share_open (const char *host, const int port, const int max_initial_buf,
		const char *journal_dir)
{
	pid_t pid;
	...
	/* Make a socket for foreground and background processes to communicate. */
		...
		)
{
	...
	if (len >= 3)
	{
		...
		if (opt)
		{
			ASSERT (pre_unmasquerade_link_buffer (buf, 2, opt));
			return p[0] == 0 && p[1] >= 14
				&& opt->op_code == (P_CONTROL_HARD_RESET_CLIENT_V2 << P_OPCODE_SHIFT);
		}
		else
#endif
			return p[0] == 0 && p[1] >= 14
