...
/*
 * Define verify_align function, otherwise it will be a noop.
 */
...
static inline int 
buffer_cmp (struct buffer *src, struct buffer *dst)
{
	ASSERT (src && dst);
	...
	{
		return memcmp (src->data + src->offset, dst->data + dst->offset, src->len);
	}
}
...
struct gc_entry
{
	...
	char padding[16 - sizeof (struct gc_entry *)];
};
...
struct gc_arena
{
	...
	bool mp;                      /** 多线程支持, 使用pthread_mutex_t锁 */
};
...
buf_set_tracking (struct buffer *buf, const uint32_t tracking)
{
	if (buf)
	{
		buf->tracking |= tracking;
		return buf->tracking;
	}
	else
		return 0;
...
/*
 * printf append to a buffer with overflow check, due to usage of vsnprintf,
 * it will leave space for a final null character and thus use only capacity - 1
 */
...
{
	return buf_valid (buf) && buf_size_valid (len) && buf->offset + buf->len + len <= buf->capacity;
}
...
const char *string_mod_const (const char *str, const unsigned int inclusive, const unsigned int exclusive,
...
# define verify_align_4(ptr) valign4 (ptr, __FILE__, __LINE__)
#else
...
static inline
void gc_init (struct gc_arena *a, bool mp)
{
	a->list = NULL;
	a->mp = mp;
}
...
char** string_array_alloc (const int array_len, const int max_string_len, struct gc_arena *gc);
void string_array_reset (char *string_array[], const int array_len, bool do_free);

int string_to_array (const char *text, unsigned char delim, char *string_array[], const int array_len,
		const int max_string_len);
void array_to_string (const char *string_array[], int array_len, const char *delim, char *text, int text_len);
