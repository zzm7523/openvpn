...
static pthread_mutex_t *static_msg_mutex = NULL;	/* 消息锁 */
...
void x_msg_va (const unsigned int flags, const char *format, va_list arglist)
{
	...
	if (!(flags & M_MSG_VIRT_OUT))
	{
		...
		else
		{
			...
			/* 输入流、输出流、文件访问需要同步, 不能用MUTEX_LOCK宏, 失败时递归 */
			if (static_msg_mutex)
	...
	if (flags & M_FATAL)
	{
		...
		// unix like系统下, 生成core文件; win32系统下, 生成mini dump文件
		...
		{
			// 主线程立即调用openvpn_exit ()退出
			openvpn_exit (OPENVPN_EXIT_STATUS_ERROR);
		}
		else
		{
			// 其它线程设置信号, 通过信号在主线程调用openvpn_exit ()退出
