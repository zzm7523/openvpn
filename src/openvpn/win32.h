...
/* Win32-specific OpenVPN code, targetted at the mingw development environment. */
...
WCHAR * wide_string (const char *utf8_str, struct gc_arena *gc);
/* Convert a string from UTF-8 to Local */
char * local_string (const char *utf8_str, struct gc_arena *gc);
/* Convert a string from Local to UTF-8 */
char * utf8_string (const char *local_str, struct gc_arena *gc);

bool win_wfp_init_funcs (void);
bool win_wfp_block_dns (const NET_IFINDEX index);
bool win_wfp_uninit (void);
...
void enableMiniDump (const char *dumpFileName);
