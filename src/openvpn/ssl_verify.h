...
{
	return (session && session->opt && (session->opt->auth_user_pass_verify_script
 ...
 *
 */
void verify_user_pass (struct user_pass *up, struct peer_info *pi, struct tls_multi *multi, struct tls_session *session);
...
void tls_multi_auth_set_client_reason (const struct plugin_return *pr, struct tls_multi *multi);
