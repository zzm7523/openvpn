...
int openvpn_execve (const struct argv *a, const struct env_set *es, const unsigned int flags, struct buffer *out);
...
struct peer_info* parse_peer_info (const char *peer_info_str);
void free_peer_info (struct peer_info *peer_info);

const struct peer_info* get_peer_info (const struct peer_info *peer_info, const char *name);
const char* get_peer_info_value (const struct peer_info *peer_info, const char *name);
...
#define GET_USER_PASS_SENSITIVE     (1<<1)  /* not used anymore */
...
static inline bool get_user_pass (struct user_pass *up, const char *auth_file, const char *prefix,
...
void argv_init_ex (struct argv *a, int argc, char *argv[]);
