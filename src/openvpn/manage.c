...
man_help (void)
{
	...
	msg (M_CLIENT, "password type p        : Enter password p for a queried " PACKAGE_NAME " password.");
	...
	msg (M_CLIENT, "pid                    : Show process ID of the current " PACKAGE_NAME " process.");
#ifdef ENABLE_PKCS11
	...
	msg (M_CLIENT, "pk-sig                 : Enter an PK signature in response to >PK_SIGN challenge");
	...
	msg (M_CLIENT, "username type u        : Enter username u for a queried " PACKAGE_NAME " username.");
...
{
	msg (M_CLIENT, ">INFO:" PACKAGE_NAME " Management Interface Version %d -- type 'help' for more info",
		MANAGEMENT_VERSION);
...
# define AF_DID_RESET (1<<1)
	...
	ASSERT (is_main_thread ());	/* 只有主线程能访问recursive_level */
#endif
	...
	if (!recursive_level) /* don't allow recursion */
	{
		...
		CLEAR (e);
		gettimeofday (&now_tv, NULL);	// 日志采用系统时间
		e.timestamp = now_tv.tv_sec;
		e.u.msg_flags = flags;
		e.string = str;
...
man_bytecount_output_client (struct management *man)
{
	...
	man->connection.bytecount_last_update = now_sec (MAIN_THREAD_INDEX);
}
		...
		struct man_def_auth_context *mdac)
{
	...
	mdac->bytecount_last_update = now_sec (MAIN_THREAD_INDEX);
}
...
man_kill (struct management *man, const char *victim)
{
	...
	if (man->persist.callback.kill_by_cn && man->persist.callback.kill_by_addr)
	{
		...
		buf_set_read (&buf, (uint8_t *) victim, (int) strlen (victim) + 1);
	...
	if (needed)
	{
		...
		if (streq (man->connection.up_query_type, type))
		{
			...
			msg (M_CLIENT, "ERROR: %s of type '%s' entered, but we need one of type '%s'",
				prompt, type, man->connection.up_query_type);
	}
	else
	{
...
{
	const bool needed = (man->connection.up_query_mode == UP_QUERY_PASS || man->connection.up_query_mode == UP_QUERY_USER_PASS)
...
man_client_auth (struct management *man, const char *cid_str, const char *kid_str, const bool extra)
{
	...
	mc->in_extra_cid = 0;
	mc->in_extra_kid = 0;
	if (parse_cid (cid_str, &mc->in_extra_cid) && parse_kid (kid_str, &mc->in_extra_kid))
	{
...
static void
man_pk_sig (struct management *man)
{
	struct man_connection *mc = &man->connection;
	if (mc->ext_key_state == EKS_SOLICIT)
	{
		...
		msg (M_CLIENT, "ERROR: The pk-sig command is not currently available");
}
...
man_dispatch_command (struct management *man, struct status_output *so, const char **p, const int nparms)
{
	...
	{
		msg (M_CLIENT, PACKAGE_NAME " Version: %s", title_string);
...
man_new_connection_post (struct management *man, const char *description)
{
	...
	set_nonblock ((int) man->connection.sd_cli);
	set_cloexec ((int) man->connection.sd_cli);
...
man_listen (struct management *man)
{
	...
	if (man->connection.sd_top == SOCKET_UNDEFINED)
	{
		...
		set_nonblock ((int) man->connection.sd_top);
		set_cloexec ((int) man->connection.sd_top);
...
man_connect (struct management *man)
{
	...
	if (status)
	{
		...
		if (man->settings.flags & MF_UNIX_SOCK)
		{
			msg (D_LINK_ERRORS, "MANAGEMENT: connect to unix socket %s failed: %s",
				sockaddr_unix_name (&man->settings.local_unix, "NULL"),
				strerror_ts (status, &gc));
		}
		else
#endif
			msg (D_LINK_ERRORS, "MANAGEMENT: connect to %s failed: %s",
				print_sockaddr (&man->settings.local, &gc),
				strerror_ts (status, &gc));
		throw_signal_soft (SIGTERM, "management-connect-failed");
		goto done;
...
man_read (struct management *man)
{
	...
	len = recv (man->connection.sd_cli, (char *) buf, sizeof (buf), MSG_NOSIGNAL);
#endif
...
#endif
			sent = send (man->connection.sd_cli, BSTR (buf), len, MSG_NOSIGNAL);
...
static void
man_persist_init (struct management *man, const int log_history_cache,
		const int echo_buffer_size, const int state_buffer_size)
{
		...
		const unsigned int flags)
{
	if (!ms->defined)
	{
...
#endif
		{
			...
			/* Run management over tunnel, or separate channel? */
			...
			{
				ms->local.addr.in4.sin_addr.s_addr =
					getaddr (GETADDR_RESOLVE|GETADDR_WARN_ON_SIGNAL|GETADDR_FATAL, addr, 0, NULL, NULL);
			}
		}
...
man_connection_init (struct management *man)
{
	...
	{
#ifdef WIN32
		/* This object is a sort of TCP/IP helper for Windows. */
		 ...
		 */
		{
			int maxevents = 2;	// 加 1, 实现wakeup功能 
		...
		const unsigned int flags)
{
	...
	/* Save the settings only if they have not been saved before. */
		...
		const in_addr_t tun_remote_ip)
{
	if (man->persist.state && (!(man->settings.flags & MF_SERVER) || state < OPENVPN_STATE_CLIENT_BASE))
	{
		...
		CLEAR (e);
		e.timestamp = now_sec (MAIN_THREAD_INDEX);
		e.u.state = state;
		e.string = detail;
...
man_output_peer_info_env (struct management *man, struct man_def_auth_context *mdac)
{
	...
	if (man->persist.callback.get_peer_info)
	{
		...
		{
			struct buffer buf;
			buf_set_read (&buf, (const uint8_t *) peer_info, (int) strlen (peer_info));
			while (buf_parse (&buf, '\n', line, sizeof (line)))
			{
				chomp (line);
...
void
management_socket_set (struct management *man, struct event_set *es, void *arg, unsigned int *persistent)
{
	if (man->connection.state != MS_INITIAL)
	{
...
man_block (struct management *man, volatile int *signal_received, const time_t expire)
{
	...
	if (man_standalone_ok (man))
	{
		while (true)
		{
			...
			tv.tv_usec = 0;
			tv.tv_sec = 1;
			status = event_wait (man->connection.es, &tv, &esr, 1, &wakeup);
			update_time (MAIN_THREAD_INDEX);
			if (man_check_for_signals (signal_received))
			{
				status = -1;
			...
			if (status > 0)
				break;
			else if (expire && now_sec (MAIN_THREAD_INDEX) >= expire)
			{
...
management_event_loop_n_seconds (struct management *man, int sec)
{
	if (man_standalone_ok (man))
	{
		...
		update_time (MAIN_THREAD_INDEX);
		if (sec)
			expire = now_sec (MAIN_THREAD_INDEX) + sec;
		...
		const char *static_challenge)
{
	...
	if (man_standalone_ok (man))
	{
		...
		/* Transfer u/p to return object, zero any record we hold in the management object. */
		if (ret)
		{
...
management_query_pk_sig (struct management *man, const char *b64_data)
{
	...
	if (man_standalone_ok (man))
	{
		...
		buf_printf (&alert_msg, ">PK_SIGN:%s", b64_data);
	...
	if (mc->ext_key_state == EKS_READY && ret)
		msg (M_CLIENT, "SUCCESS: pk-sig command succeeded");
	else if (mc->ext_key_state == EKS_INPUT || mc->ext_key_state == EKS_READY)
		msg (M_CLIENT, "ERROR: pk-sig command failed");
...
/*
 * Return true if (from the management interface's perspective) OpenVPN should daemonize.
 */
bool
