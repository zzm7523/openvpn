...
#define KEY_EXPANSION_ID "OpenVPN"	/* 为了兼容OpenVPN不能改为 PACKAGE_NAME */
...
/*
 * If TLS_AGGREGATE_ACK, set the max number of acknowledgments that
 * can "hitch a ride" on an outgoing non-P_ACK_V1 control packet.
 */
 ...
 * interprocess communication pipe to the main thread to be ready to accept writes.
 */
...
#define TLS_OPTIONS_LEN	4096 /* TLS_OPTIONS_LEN >= (2 * MAX_ALGORITHM_STRING_LEN) */
...
#define RENEGOTIATE_TIME_MIN 60
...
bool require_negotiate_algorithm (struct context *c);

bool require_negotiate_tunnel_options (struct context *c);
 ...
 */
struct tls_multi *tls_multi_init (struct context *c, struct tls_options *tls_options);
 ...
 */
bool tls_pre_decrypt (struct context *c, struct packet_buffer *buf);
 ...
 */
bool tls_pre_decrypt_lite (const struct tls_auth_standalone *tas, const struct link_socket_actual *from,
 ...
 * reason string and return a dynamically allocated auth_challenge_info struct.
 */
...
tls_test_payload_len (const struct tls_multi *multi)
{
	if (multi)
	{
		const struct key_state *ks = multi->session[TM_ACTIVE].ks_primary;
