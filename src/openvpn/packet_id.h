...
/*
 * These routines are designed to catch replay attacks, where a man-in-the-middle captures packets
 * and then attempts to replay them back later.
 */
...
#if 1
/*
 * These are the types that members of a struct packet_id_net are converted to for network transmission.
 */
...
/*
 * In TLS mode, when a packet ID gets to this level, start thinking about triggering a new SSL/TLS handshake.
 */
#define PACKET_ID_WRAP_TRIGGER 0xF8000000	/* 从0xFF000000更改为0xF8000000 */
...
/*
 * DEBUGGING ONLY.
 * Make packet_id_type and net_time_t small to test wraparound logic and corner cases.
 */
...
/*
 * Maximum allowed backtrack in sequence number due to packets arriving out of order.
 */
#define MIN_SEQ_BACKTRACK 0
#define MAX_SEQ_BACKTRACK 65536
#define DEFAULT_SEQ_BACKTRACK 128	/* 从64更改为128 */

/*
 * Maximum allowed backtrack in seconds due to packets arriving out of order.
 */
...
/*
 * Do a reap pass through the sequence number array once every n seconds in order to
 * expire sequence numbers which can no longer be accepted because they would violate TIME_BACKTRACK.
 */
...
/*
 * This is the data structure we keep on the receiving side, to check that no packet-id
 * (i.e. sequence number + optional timestamp) is accepted more than once.
 */
...
/*
 * file to facilitate cross-session persistence of time/id
 */
...
/*
 * Communicate packet-id over the wire. A short packet-id is just a 32 bit sequence number.
 * A long packet-id includes a timestamp as well.
 *
 * Long packet-ids are used as IVs for CFB/OFB/CTR ciphers.
 *
 * This data structure is always sent over the net in network byte order, by calling htonpid, ntohpid,
 * htontime, and ntohtime on the data elements to change them to and from standard sizes.
 *
 * In addition, time is converted to a net_time_t before sending, since openvpn always
 * uses a 32-bit time_t but some 64 bit platforms use a 64 bit time_t.
 */
...
/*
 * Keep a record of our current packet-id state on the sending side.
 */
...
void packet_id_init (struct packet_id *p, bool tcp_mode, int seq_backtrack, int time_backtrack,
		const char *name, int unit);
void packet_id_free (struct packet_id *p);

/* should we accept an incoming packet id ? */
bool packet_id_test (struct packet_id_rec *p, time_t now_sec, const struct packet_id_net *pin);

/* change our current state to reflect an accepted packet id */
void packet_id_add (struct packet_id_rec *p, time_t now_sec, const struct packet_id_net *pin);

/* expire TIME_BACKTRACK sequence numbers */ 
void packet_id_reap (struct packet_id_rec *p, time_t now_sec);
 ...
 * Sequence number ranges from 1 to 2^32 - 1. In long_form, a time_t is added as well.
 */
static inline bool
packet_id_alloc_outgoing (struct packet_id_send *p, struct packet_id_net *pin, bool long_form, time_t now_sec)
{
	if (!p->time)
		p->time = now_sec;
	...
	pin->time = p->time;
	pin->id = ++p->id;
	if (pin->id == PACKET_ID_MAX)
	{
		if (!long_form || now_sec <= p->time)
			return false;
		else
		{
			p->time = now_sec;
...
static inline bool
check_timestamp_delta (time_t remote, time_t now_sec, unsigned int max_delta)
{
	unsigned int abs;
	const time_t local_now = now_sec;

	if (local_now >= remote)
		abs = (unsigned int) (local_now - remote);
	else
		abs = (unsigned int) (remote - local_now);

	return abs <= max_delta;
}

static inline void
packet_id_reap_test (struct packet_id_rec *p, time_t now_sec)
{
	if (p->last_reap + SEQ_REAP_INTERVAL <= now_sec)
		packet_id_reap (p, now_sec);
}
