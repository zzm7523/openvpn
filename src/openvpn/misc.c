			 ...
			 struct env_set *es)
{
	...
	if (command)
	{
		...
		argv_printf (&argv, "%sc %s %d %d %s %s %s",
			command,
			arg,
			tun_mtu, link_mtu,
			ifconfig_local, ifconfig_remote,
...
openvpn_execve_check (const struct argv *a, const struct env_set *es, const unsigned int flags, const char *error_message)
{
	struct gc_arena gc = gc_new ();
	int ret = false;
	const int stat = openvpn_execve (a, es, flags, NULL);
	...
	{
		if (error_message)
			msg (((flags & S_FATAL) ? M_FATAL : M_WARN), "%s: %s", error_message, system_error_message (stat, &gc));
	}
	gc_free (&gc);
	return ret;
}
...
openvpn_execve (const struct argv *a, const struct env_set *es, const unsigned int flags, struct buffer *out)
{
	...
	if (a && a->arg_v[0])
	{
		...
		{
			const char *cmd = a->arg_v[0];
			char *const *arg_v = a->arg_v;
...
env_set_print (int msglevel, const struct env_set *es)
{
	if (check_debug_level (msglevel))
	{
		...
		if (es)
		{
			pthread_mutex_lock ((pthread_mutex_t*) (&g_env_mutex));
			e = es->list;
			i = 0;
			...
			pthread_mutex_unlock ((pthread_mutex_t*) (&g_env_mutex));
		}
	}
}
...
create_temp_file (const char *directory, const char *prefix, struct gc_arena *gc)
{
	...
	do
	{
		...
		prng_bytes (rndbytes, sizeof (rndbytes), MAIN_THREAD_INDEX);
		...
		/* Atomically create the file.  Errors out if the file already exists.  */
		...
		else if (fd == -1 && errno != EEXIST)
		{
			...
			msg (M_FATAL, "Could not create temporary file '%s': %s", retfname, strerror_ts (errno, &gcerr));
			gc_free (&gcerr);
			return NULL;
		}
	} while (attempts < 6);
...
#define N_RND_BYTES 6
	...
	prng_bytes (rnd_bytes, sizeof (rnd_bytes), MAIN_THREAD_INDEX);
...
#ifdef WIN32
		return c == '\\' || (isalpha (c) && pathname[1] == ':' && (pathname[2] == '\\' || pathname[2] == '/'));
#else
		return c == '/';
#endif
	}
...
bool
get_user_pass_cr (struct user_pass *up, const char *auth_file, const char *prefix, const unsigned int flags,
...
get_auth_challenge (const char *auth_challenge, struct gc_arena *gc)
{
	...
	if (auth_challenge)
	{
		...
		const size_t len = strlen (auth_challenge);
		char *work = (char *) gc_malloc (len + 1, false, gc);
		char *cp;

		struct buffer b;
		buf_set_read (&b, (const uint8_t *) auth_challenge, (int) len);

		ALLOC_OBJ_CLEAR_GC (ac, struct auth_challenge_info, gc);

		/* parse prefix */
		if (!buf_parse (&b, ':', work, (int) len))
			return NULL;
		if (strcmp (work, "CRV1"))
			return NULL;
...
make_inline_array (const char *str, struct gc_arena *gc)
{
	...
	buf_set_read (&buf, (const uint8_t *) str, (int) strlen (str));
	while (buf_parse (&buf, '\n', line, sizeof (line)))
		++len;
...
/* Return the next largest power of 2 or u if u is a power of 2. */
size_t
...
{
	const size_t newarg_c = a->arg_c + add + 1;

	ASSERT (newarg_c > a->arg_c);
	argv_extend (a, adjust_power_of_2 (newarg_c));
}
...
void
argv_init_ex (struct argv *a, int argc, char *argv[])
{
	int i;
	...
	{
		if (argv[i])
			argv_append (a, string_alloc (argv[i], NULL));
	}
}
...
argv_clone (const struct argv *a, const size_t headroom)
{
	...
	if (a)
	{
		r.norm = a->norm;
		for (i = 0; i < a->arg_c; ++i)
			argv_append (&r, string_alloc (a->arg_v[i], NULL));
...
argv_insert_head (const struct argv *a, const char *head)
{
	...
	r = argv_clone (a, 1);
	r.arg_v[0] = string_alloc (head, NULL);
	s = r.system_str;
...
{
	if (a->arg_v)
		return print_argv ((const char **)a->arg_v, gc, flags);
	else
		return "";
}
...
/* peer_info_string×Ö·û´®ÊäÈëÀàËÆ, IV_VER=5.0.0\nIV_PLAT=windows7\nIV_HWADDR=4c:eb:42:74:64:4a\n */

struct peer_info*
parse_peer_info (const char *peer_info_str)
{
#define MAX_INFO_LENGTH	256
#define MAX_INFO_ITEM	64

	struct peer_info *head = NULL, *curr = NULL, *next = NULL;

	if (peer_info_str && strlen (peer_info_str) > 0)
	{
		struct gc_arena gc = gc_new ();
		const char *name = NULL, *value = NULL;
		int i = 0, array_len = MAX_INFO_ITEM;
		char **string_array = string_array_alloc (MAX_INFO_ITEM, MAX_INFO_LENGTH, &gc);

		array_len = string_to_array (peer_info_str, '\n', string_array, MAX_INFO_ITEM, MAX_INFO_LENGTH);

		for (i = 0; i < array_len; ++i)
		{
			if (deconstruct_name_value (string_array[i], &name, &value, &gc))
			{
				ALLOC_OBJ_CLEAR (curr, struct peer_info);

				name = skip_leading_whitespace (name);
				curr->name = string_alloc (name, NULL);
				value = skip_leading_whitespace (value);
				curr->value = string_alloc (value, NULL);
...
void
free_peer_info (struct peer_info *peer_info)
{
	struct peer_info *curr = peer_info, *next = NULL;
...
const struct peer_info*
get_peer_info (const struct peer_info *peer_info, const char *name)
{
	const struct peer_info *next = peer_info;

	while (next)
	{
		if (0 == strcmp (name, next->name))
			return next;
		else
			next = next->next;
	}
...
const char*
get_peer_info_value (const struct peer_info *peer_info, const char *name)
{
	const struct peer_info *cand = get_peer_info (peer_info, name);
	return cand ? cand->value : NULL;
}

struct version_info get_peer_version_info (const struct peer_info *peer_info)
{
	const char *version_str = get_peer_info_value (peer_info, "IV_VER");
	...
	if (version_str)
	{
#define MAX_VERSION_ITEM	3
#define MAX_VERSION_LENGTH	64

		struct gc_arena gc = gc_new ();
		int array_len = MAX_VERSION_ITEM;
		char **string_array = string_array_alloc (MAX_VERSION_ITEM, MAX_VERSION_LENGTH, &gc);

		array_len = string_to_array (version_str, '.', string_array, MAX_VERSION_ITEM, MAX_VERSION_LENGTH);

		if (array_len > 0)
			ver_info.major = atoi (string_array[0]);
		if (array_len > 1)
			ver_info.minor = atoi (string_array[1]);
