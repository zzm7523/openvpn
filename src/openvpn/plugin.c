...
# define PLUGIN_SYM(var, name, flags) dll_resolve_symbol (p->module, (void**) &p->var, name, p->so_pathname, flags)
	...
	*/
	...
	if (rel)
		msg (M_WARN, "WARNING: plugin '%s' specified by a relative pathname -- using an absolute pathname would be more secure",
			p->so_pathname);
...
static void
plugin_open_item (struct plugin *p, const struct plugin_option *o, struct openvpn_plugin_string_list **retlist,
		const char **envp, const int init_point)
{
	...
	if (!p->plugin_handle && init_point == p->requested_initialization_point)
	{
		...
		if (p->open3)
		{
			struct openvpn_plugin_args_open_in args = {
				p->plugin_type_mask,
...
static int
plugin_call_item (const struct plugin *p, void *per_client_context, const int type, const struct argv *av,
		struct openvpn_plugin_string_list **retlist, const char **envp
#ifdef ENABLE_SSL
		, int certdepth, openvpn_x509_cert_t *current_cert
#endif
		 )
{
	...
	if (p->plugin_handle && (p->plugin_type_mask & OPENVPN_PLUGIN_MASK (type)))
	{
		...
		plugin_show_args_env (D_PLUGIN_DEBUG, (const char **) a.arg_v, envp);
		...
		*/
		if (p->func3)
		{
			...
			status = (*p->func2) (p->plugin_handle, type, (const char **)a.arg_v, envp, per_client_context, retlist);
		else if (p->func1)
			status = (*p->func1) (p->plugin_handle, type, (const char **)a.arg_v, envp);
		else
			ASSERT (0);
...
static void
plugin_per_client_init (const struct plugin_common *pc, struct plugin_per_client *cli, const int init_point)
{
	const int n = pc->n;
	int i;
...
static void
plugin_common_open (struct plugin_common *pc, const struct plugin_option_list *list, struct plugin_return *pr,
		const struct env_set *es, const int init_point)
{
	struct gc_arena gc = gc_new ();
	int i;
	const char **envp;
	...
	for (i = 0; i < pc->n; ++i)
		plugin_open_item (&pc->plugins[i], &list->plugins[i], pr ? &pr->list[i] : NULL, envp, init_point);
...
void
plugin_list_open (struct plugin_list *pl, const struct plugin_option_list *list, struct plugin_return *pr,
		const struct env_set *es, const int init_point)
{
...
void
plugin_return_get_column (const struct plugin_return *src, struct plugin_return *dest, const char *colname)
{
	int i;
