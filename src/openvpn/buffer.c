...
static pthread_mutex_t *static_gc_mutex = NULL;	/* gc_area 内存分配锁 */
...
#endif
{
	struct buffer ret = { 0x0 };
	ret.capacity = buf->capacity;
	ret.offset = buf->offset;
	ret.len = buf->len;
	ret.tracking = buf->tracking;
#ifdef DMALLOC
...
/*
 * write a string to the end of a buffer that was truncated by buf_printf
 */
void
...
{
	const size_t len = strlen ((char *) BPTR (buf));
...
void gc_mutex_init ()
{
	if (!static_gc_mutex)
	{
		static_gc_mutex = (pthread_mutex_t *) malloc (sizeof (pthread_mutex_t));
		ASSERT (0 == pthread_mutex_init (static_gc_mutex, NULL));
	}
}

void gc_mutex_destory ()
{
	if (static_gc_mutex)
	{
		pthread_mutex_destroy (static_gc_mutex);
		static_gc_mutex = NULL;
	}
}
...
#endif
{
	...
	/**
	 * !! struct gc_entry结构的大小必须是16的整数倍 !!
	 * !! x86_64 SSE 指令集操作的内存必须是以16位字节边界对齐的, 编译器会使用SSE指令集优化对
	 * __declspec(align(CACHE_LINE_SIZE)), __attribute__((__aligned__(CACHE_LINE_SIZE))) 修饰的结构的操作 
	 */
	ASSERT ((sizeof (struct gc_entry) & (16 - 1)) == 0);
	...
	if (a)
	{
		...
		if (a->mp)
		{
			ASSERT (0 == pthread_mutex_lock (static_gc_mutex));
			e->next = a->list;
			a->list = e;
			ASSERT (0 == pthread_mutex_unlock (static_gc_mutex));
		}
		else
		{
			e->next = a->list;
...
char *
format_hex_ex (const uint8_t *data, int size, int maxoutput, int space_break, const char *separator,
		struct gc_arena *gc)
{
	struct buffer out = alloc_buf_gc (maxoutput ? maxoutput :
		((size * 2) + (size / space_break) * (int) strlen (separator) + 2), gc);
	int i;
	for (i = 0; i < size; ++i)
	{
...
/*
 * force a null termination even it requires truncation of the last char.
 */
void
...
/*
 * Remove trailing \r and \n chars and ensure null termination.
 */
void
...
bool string_array_has (const char **array, const char *str)
{
	int i = 0;
	if (array && str)
	{
...
#ifdef DMALLOC
	buf_set_read (&buf, (uint8_t*) string_alloc_debug (str, gc, file, line), (int) strlen (str) + 1);
#else
	buf_set_read (&buf, (uint8_t*) string_alloc (str, gc), (int) strlen (str) + 1);
#endif
...
char** 
string_array_alloc (const int array_len, const int max_string_len, struct gc_arena *gc)
{
	char **string_array = (char**) gc_malloc (array_len * sizeof (char*), true, gc);

	if (string_array)
	{
		int i;
		for (i = 0; i < array_len; ++i)
			string_array[i] = (char*) gc_malloc (max_string_len + 2, true, gc);
	}
...
void 
string_array_reset (char *string_array[], const int array_len, bool do_free)
{
	int i;

	for (i = 0; i < array_len; ++i)
	{
		if (do_free && string_array[i])
			free (string_array[i]);
		string_array[i] = NULL;
	}
}
...
int 
string_to_array (const char *text, unsigned char delim, char **string_array, const int array_len,
		const int max_string_len)
{
	int i = 0;

	if (text && string_array && array_len > 0)
	{
		const char *curr = text;
		const char *pos = NULL;

		while (curr != NULL && i < array_len)
		{
			if ((pos = strchr ((char*) curr, delim)) != NULL)
			{
				if (pos - curr < max_string_len)
				{
					strncpy (string_array[i], curr, pos - curr);
					*(string_array[i++] + (pos - curr)) = 0x0;
					curr = pos + 1;
				}
				else
					return 0;
			...
			{	
				if (strlen (curr) < (size_t) max_string_len)
				{
					strcpy (string_array[i], curr);
					*(string_array[i++] + strlen (curr)) = 0x0;
					break;
				}
				else
					return 0;
...
void 
array_to_string (const char *string_array[], int array_len, const char *delim, char *text, int text_len)
{
	int i = 0, len = 0, residual_len = text_len;
	char *p = text;

	text[text_len - 1] = 0x0;	// 空终止

	for (i = 0; i < array_len; ++i)
	{
		if (string_array[i])
		{
			len = snprintf (p, residual_len, "%s%s", i == 0 ? "" : delim, string_array[i]);
			if (len > 0)
			{
				residual_len -= len;
				p += len;
