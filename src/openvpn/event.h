...
/*
 * rwflags passed to event_ctl and returned by struct event_set_return.
 */
...
struct event_set_functions
{
	 ...
	 */
	int  (*wait)(struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen, bool *wakeup);
	void (*wakeup)(struct event_set *es);
};
...
struct event_set_return
{
	...
	void *arg;
	char padding[CACHE_LINE_SIZE - sizeof (unsigned int) - sizeof (void *)];
};
...
static inline int
event_wait (struct event_set *es, struct timeval *tv, struct event_set_return *out, int outlen, bool *wakeup)
{
	int ret;
	perf_push (PERF_IO_WAIT);
	ret = (*es->func.wait)(es, tv, out, outlen, wakeup);
	perf_pop ();
	return ret;
}

static inline void
event_wakeup (struct event_set *es)
{
	(*es->func.wakeup)(es);
}
