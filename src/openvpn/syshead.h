...
millisleep (unsigned int milliseconds)
{
	struct timespec rqtp;
	rqtp.tv_sec = milliseconds / 1000;
	rqtp.tv_nsec = (milliseconds % 1000) * 1000;
	return nanosleep (&rqtp, 0 /*NULL*/);
}
#endif
...
/*
 * Does this platform support linux-style IP_PKTINFO or bsd-style IP_RECVDSTADDR ?
 */
...
/*
 * Does this platform define SOL_IP or only bsd-style IPPROTO_IP ?
 */
#ifndef SOL_IP
...
/*
 * Define a boolean value based on Win32 status.
 */
#ifdef WIN32
#define WIN32_0_1 1
#else
...
/*
 * Should we enable the use of execve() for calling subprocesses, instead of system()?
 */
...
/*
 * MANAGEMENT_IN_EXTRA allows the management interface to read multi-line inputs from clients.
 */
...
/*
 * 隧道参数协商要求OCC支持, 所以不能再启用ENABLE_SMALL了
 */
#ifdef ENABLE_SMALL
#error "Enable smaller executable size no longer supported!"
#endif
...
/*
 * 运行时一致性检查功能只能在调试模式下开启(严重影响系统性能)
 */
#ifndef _DEBUG
#if defined(PACKET_BUFFER_LIST_CHECK)
#error PACKET_BUFFER_LIST_CHECK can only be used in debug mode
#endif
#if defined(PACKET_TRUNCATION_CHECK)
#error PACKET_TRUNCATION_CHECK can only be used in debug mode
#endif
#if defined(THREAD_ACCESS_CHECK)
#error THREAD_ACCESS_CHECK can only be used in debug mode
#endif
#if defined(HASH_TABLE_CHECK)
#error HASH_TABLE_CHECK can only be used in debug mode
#endif
#if defined(PERF_STATS_CHECK)
#error PERF_STATS_CHECK can only be used in debug mode
#endif
#endif

#define MAX_THREAD_GROUP    4

#define MAIN_THREAD_INDEX   0
#ifndef ENABLE_TUN_THREAD
#define TUN_THREAD_INDEX    0
#else
#define TUN_THREAD_INDEX    1
#endif

#define WORKER_THREAD_INDEX_BASE  2
#define MIN_WORKER_THREAD   1  /* 工作线程数必须大于等于1 */
#define MAX_WORKER_THREAD   8
#define MAX_THREAD_INDEX    (WORKER_THREAD_INDEX_BASE + MAX_WORKER_THREAD)

 /* 超时值不能太大(xxx_bufs->size, xxx_pendings->size没有同步访问); 也不能太小(频繁唤醒进程); 2秒 */
#define WORKER_THREAD_WAIT_TIMEOUT  2

#define MIN_WORK_CHUNK_SIZE   2
#define MAX_WORK_CHUNK_SIZE   6

 /* !! 不要定义的太大, 读写需要交替进行; 否则可能触发系统的流控机制 */
#define MAX_LINK_BATCH_READ   8
#define MAX_LINK_BATCH_WRITE  8

#define MAX_TUN_BATCH_READ    8
#define MAX_TUN_BATCH_WRITE   8

#define MAX_PROCESS_IO_LOOP   16

#define RECLAIM_THRESHOLD     32
...
#ifdef WIN32
#define __CACHE_LINE_ALIGNED__ __declspec(align(CACHE_LINE_SIZE))
#else
...
#define THREAD_SELF_INDEX() get_thread_index (pthread_self ())
