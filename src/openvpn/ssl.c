...
#ifdef ENABLE_GUOMI
	{"RSA-SM1-SHA1", "RSA-SM1-SHA1"},
	{"RSA-SM1-SM3", "RSA-SM1-SM3"},
	{"ECC-SM1-SHA1", "ECC-SM1-SHA1"},
	{"ECC-SM1-SM3", "ECC-SM1-SM3"},
	{"ECDHE-SM1-SHA1", "ECDHE-SM1-SHA1"},
	{"ECDHE-SM1-SM3", "ECDHE-SM1-SM3"},
	{"ECDHE-SM4-SM3", "ECDHE-SM4-SM3"},
	{"ECC-SM4-SM3", "ECC-SM4-SM3"},
	{"RSA-SM4-SHA1", "RSA-SM4-SHA1"},
	{"RSA-SM4-SM3", "RSA-SM4-SM3"},
#endif
	...
	if (cipher && (cipher_kt_block_size (cipher) < 128 / 8))
	{
		if (*reneg_bytes == -1) /* Not user-specified */
		{
			msg (M_WARN, "WARNING: cipher with small block size in use, "
				"reducing reneg-bytes to 256MB to mitigate SWEET32 attacks.");
			*reneg_bytes = 256 * 1024 * 1024;
		}
	}
}

/*
 * Max number of bytes we will add for data structures common to both
 * data and control channel packets. (opcode only). 
 */
void
...
/*
 * Max number of bytes we will add to control channel packet. 
 */
static void
tls_init_control_channel_frame_parameters (const struct frame *data_channel_frame, struct frame *frame)
{
	/* frame->extra_frame is already initialized with tls_auth buffer requirements, if --tls-auth is enabled. */
...
bool
require_negotiate_algorithm (struct context *c)
{
#ifdef ENABLE_OCC
	/* key_method == 2 支持算法协商 */
	if (c && c->options.key_method == 2)
	{
		struct tls_multi *multi = c->c2.tls_multi;

		if (multi && multi->opt.ciphername_defined && multi->opt.authname_defined)
		{
			/* 上下文已经完成了算法协商 */
			return false;
		}
		...
		{
			/* 不支持空算法协商; 单一算法也要求协商(协商隧道参数) */
			return c->options.ciphername_defined && c->options.authname_defined;
		}
	}
	else
	{
...
bool
require_negotiate_tunnel_options (struct context *c)
{
#ifdef ENABLE_OCC
...
/*
 * OpenSSL library calls pem_password_callback if the private key is protected by a password.
 */
...
bool auth_user_pass_enabled;     /* GLOBAL */
struct user_pass auth_user_pass; /* GLOBAL */
...
# ifdef ENABLE_CLIENT_CR
		if (auth_challenge) /* dynamic challenge/response */
			get_user_pass_cr (&auth_user_pass, auth_file, UP_TYPE_AUTH,
				GET_USER_PASS_MANAGEMENT|GET_USER_PASS_DYNAMIC_CHALLENGE, auth_challenge);
...
static
void send_tunnel_establish_error (struct tls_multi *multi, const char *error_reason)
{
	bool send_control_channel_string (struct context *c, const char *str, int msglevel);
	void schedule_exit (struct context *c, const int n_seconds, const int signal);

	static const char establish_error[] = "HALT";	// 采用HALT, 兼容老版本

	struct gc_arena gc = gc_new ();
	struct context *c = multi->c;
	size_t len;

	schedule_exit (c, c->options.scheduled_exit_interval, SIGTERM);

	len = (error_reason ? strlen (error_reason) + 1 : 0) + sizeof (establish_error);
	if (len > PUSH_BUNDLE_SIZE)
		len = PUSH_BUNDLE_SIZE;

	{
		struct buffer buf = alloc_buf_gc (len, &gc);
		buf_printf (&buf, establish_error);
		if (error_reason)
			buf_printf (&buf, ",%s", error_reason);
...
init_ssl (const struct options *options, struct tls_root_ctx *new_ctx)
{
	...
	tls_ctx_set_options (new_ctx, options->ssl_flags, options->tls_version);

	if (options->pkcs12_file)
	{
		if (!tls_ctx_load_pkcs12 (new_ctx, options->pkcs12_file, options->pkcs12_file_inline, !options->ca_file))
			goto err;
	}
#ifdef ENABLE_PKCS11
...
#endif
#ifdef ENABLE_GUOMI
	else if (options->gmedapi_cert)
	{
		tls_ctx_load_gmedapi (new_ctx, options->gmedapi_cert);
	}
#endif
#ifdef MANAGMENT_EXTERNAL_KEY
	else if ((options->management_flags & MF_EXTERNAL_KEY) && options->cert_file)
	{
		tls_ctx_use_external_private_key (new_ctx, options->cert_file, options->cert_file_inline);
	}
#endif
	else
	{
...
#ifdef ENABLE_GUOMI
	/* 加载加密证书, 硬件设备的签名证书、加密证书总是成对出现, 不用单独指定 */
	if (options->encrypt_pkcs12_file)
	{
		if (!tls_ctx_load_encrypt_pkcs12 (new_ctx, options->encrypt_pkcs12_file, options->encrypt_pkcs12_file_inline))
			goto err;
	}
	else
	{
		/* Load encrypt Certificate */
		if (options->encrypt_cert_file)
		{
			tls_ctx_load_encrypt_cert_file (new_ctx, options->encrypt_cert_file, options->encrypt_cert_file_inline);
		}

		/* Load encrypt Private Key */
		if (options->encrypt_priv_key_file)
		{
			if (0 != tls_ctx_load_encrypt_priv_file (new_ctx, options->encrypt_priv_key_file, options->encrypt_priv_key_file_inline))
				goto err;
		}
	}
#endif
...
/*
 * Given a key_method, return true if op represents the required form of hard_reset.
 *
...
key_state_init (struct tls_multi *multi, struct tls_session *session, struct key_state *ks)
{
	...
	SSL_set_ex_data (ks->ks_ssl.ssl, global_SSL_key_state_index, ks);
	...
	ks->local_key_id = multi->local_key_id;
	...
	*/
	...
	reliable_init (ks->send_reliable, BUF_SIZE (&session->opt->frame), FRAME_HEADROOM (&session->opt->frame),
		TLS_RELIABLE_N_SEND_BUFFERS, ks->key_id ? false : session->opt->xmit_hold);
	reliable_init (ks->rec_reliable, BUF_SIZE (&session->opt->frame), FRAME_HEADROOM (&session->opt->frame),
		TLS_RELIABLE_N_REC_BUFFERS, false);
	reliable_set_timeout (ks->send_reliable, session->opt->packet_timeout);

	/* init packet ID tracker */
	packet_id_init (&ks->packet_id, session->opt->tcp_mode, session->opt->replay_window, session->opt->replay_time,
		"SSL", ks->key_id);
 ...
 */
static inline void 
tls_session_set_self_referential_pointers (struct tls_session* session)
{
	session->tls_auth.packet_id = &session->tls_auth_pid;
	session->ks_primary   = &session->key[0];
	session->ks_lame_duck = &session->key[1];
	session->ks_garbage   = &session->key[2];
}
...
tls_session_init (struct tls_multi *multi, struct tls_session *session)
{
	...
	key_state_init (multi, session, session->ks_primary);
...
/*
 * Return true if "lame duck" or retiring key has expired and can no longer be used.
 */
static inline bool
lame_duck_must_die (const struct tls_session *session, interval_t *wakeup)
{
	const struct key_state *lame = session->ks_lame_duck;
	const time_t local_now = now_sec (MAIN_THREAD_INDEX);

	if (lame->error != 0)		/* 错误发生时, 返回true */
		return true;
...
struct tls_multi *
tls_multi_init (struct context *c, struct tls_options *tls_options)
{
	...
	for (i = 0; i < TM_SIZE; ++i)
	{
		ret->session[i].ks_primary   = &ret->session[i].key[0];
		ret->session[i].ks_lame_duck = &ret->session[i].key[1];
		ret->session[i].ks_garbage   = &ret->session[i].key[2];
	}

	/* set up list of keys to be scanned by data channel encrypt and decrypt routines */
	ASSERT (SIZE (ret->key_scan) == 3);
	ret->key_scan[0] = &ret->session[TM_ACTIVE].ks_primary;
	ret->key_scan[1] = &ret->session[TM_ACTIVE].ks_lame_duck;
	ret->key_scan[2] = &ret->session[TM_LAME_DUCK].ks_lame_duck;
...
void
tls_auth_standalone_finalize (struct tls_auth_standalone *tas, const struct frame *frame)
{
 ...
 * Used to verify compatibility of local and remote option sets.
 */
void
...
/*
 * Dependent on hmac size, opcode size, and session_id size. Will assert if too small.
 */
...
write_control_auth (struct tls_session *session, struct key_state *ks, struct buffer *buf,
		struct link_socket_actual **to_link_addr, int opcode, int max_ack, bool prepend_ack)
{
	struct crypto_options *opt = &session->tls_auth;
	uint8_t *header;
	...
	*header = ks->key_id | (opcode << P_OPCODE_SHIFT);
	if (opt->key_ctx_bi->encrypt.hmac)
	{
		/* no encryption, only write hmac */
		if (opt->packet_id)
		{
			struct packet_id_net pin;
			if (packet_id_alloc_outgoing (&opt->packet_id->send, &pin, BOOL_CAST (opt->flags & CO_PACKET_ID_LONG_FORM),
		...
		ASSERT (generate_hmac (MAIN_THREAD_INDEX, opt, buf));
		ASSERT (swap_hmac (buf, opt, false));
	}
 ...
 */
static bool
read_control_auth (struct buffer *buf, struct crypto_options *opt, const struct link_socket_actual *from)
{
	struct gc_arena gc = gc_new ();

	if (opt->key_ctx_bi->decrypt.hmac)
	{
		struct packet_id_net pin;
		bool have_pin = false;

		/* move the hmac record to the front of the packet */
		if (!swap_hmac (buf, opt, true))
		{
		...
		/* authenticate only (no decrypt) and remove the hmac record from the head of the buffer */
		if (!verify_hmac (MAIN_THREAD_INDEX, opt, buf))
		{
			gc_free (&gc);
			return false;
		}
		...
		if (opt->packet_id)
		{
			if (!packet_id_read (&pin, buf, BOOL_CAST (opt->flags & CO_PACKET_ID_LONG_FORM)))
			{
				msg (D_TLS_ERRORS, "Authenticate/Decrypt packet error: error reading packet-id");
				gc_free (&gc);
				return false;
			}
			else
		...
		if (have_pin)
		{
			packet_id_reap_test (&opt->packet_id->rec, now_sec (MAIN_THREAD_INDEX));
			if (packet_id_test (&opt->packet_id->rec, now_sec (MAIN_THREAD_INDEX), &pin))
			{
				packet_id_add (&opt->packet_id->rec, now_sec (MAIN_THREAD_INDEX), &pin);
			...
			else
			{
				if (!(opt->flags & CO_MUTE_REPLAY_WARNINGS))
					msg (D_REPLAY_ERRORS, "%s: bad packet ID (may be a replay): %s -- see the man page entry for --no-replay and --replay-window for more info or silence this warning with --mute-replay-warnings",
						...
						packet_id_net_print (&pin, true, &gc));
				gc_free (&gc);
				return false;
			}
		}
	...
	/* advance buffer pointer past opcode & session_id since our caller already read it */
...
key_source_print (const struct key_source *k, const char *prefix)
{
	...
	dmsg (D_SHOW_KEY_SOURCE, "%s pre_master: %s",
		prefix, format_hex (k->pre_master, sizeof (k->pre_master), 0, &gc));
	dmsg (D_SHOW_KEY_SOURCE, "%s random1: %s",
		prefix, format_hex (k->random1, sizeof (k->random1), 0, &gc));
	dmsg (D_SHOW_KEY_SOURCE, "%s random2: %s",
		prefix, format_hex (k->random2, sizeof (k->random2), 0, &gc));
 ...
 */
void
tls1_P_hash (const md_kt_t *md_kt, const uint8_t *sec, int sec_len, const uint8_t *seed, int seed_len,
 ...
 */
static void
tls1_PRF (uint8_t *label, int label_len, const uint8_t *sec, int slen, uint8_t *out1, int olen)
{
	struct gc_arena gc = gc_new ();
		...
		uint8_t *output, int output_len)
{
	...
	ASSERT (buf_write (&seed, label, (int) strlen (label)));
...
/* 
 * Using source entropy from local and remote hosts, mix into master key.
 */
static bool
generate_key_expansion (struct key_state *ks,
		...
		bool server)
{
	uint8_t master[48] = { 0x0 };
	struct key2 key2 = { 0x0 };
	bool ret = false;
	...
	/* Initialize OpenSSL key contexts */
	{
		...
		for (i = 0; i < MAX_THREAD_INDEX; ++i)
		{
			init_key_ctx (&ks->key[i].encrypt, &key2.keys[(int) server],  key_type,
				OPENVPN_OP_ENCRYPT, i == 0 ? "Data Channel Encrypt" : NULL);

			init_key_ctx (&ks->key[i].decrypt, &key2.keys[1 - (int) server], key_type,
				OPENVPN_OP_DECRYPT, i == 0 ? "Data Channel Decrypt" : NULL);
		}
	}
...
{
	struct key_state *new_ks_primary = session->ks_garbage;

	/* remaining lifetime of old key */
	session->ks_primary->must_die = now_sec (MAIN_THREAD_INDEX) + session->opt->transition_window;
	key_state_free (session->ks_garbage, false);
	session->ks_garbage   = session->ks_lame_duck;
	session->ks_lame_duck = session->ks_primary;
	session->ks_primary   = new_ks_primary;

	key_state_init (multi, session, session->ks_primary);
	session->ks_primary->session_id_remote = session->ks_lame_duck->session_id_remote;
	session->ks_primary->remote_addr = session->ks_lame_duck->remote_addr;
}
...
write_string (struct buffer *buf, const char *str, const int maxlen)
{
	...
	if (len < 1 || (maxlen >= 0 && len > maxlen))
		return false;
	if (!buf_write_u16 (buf, (int) len))
		return false;
	if (!buf_write (buf, str, (int) len))
		return false;
	return true;
}
...
/*
 * Handle the reading and writing of key data to and from the TLS control channel (cleartext).
 */
...
{
	struct key key;
	struct key_state *ks = session->ks_primary;        /* primary key */
	struct key_state *ks_lame = session->ks_lame_duck; /* retiring key */
...

	{
		...
		for (i = 0; i < MAX_THREAD_INDEX; ++i)
		{
			init_key_ctx (&ks->key[i].encrypt, &key, &session->opt->key_type,
				OPENVPN_OP_ENCRYPT, i == 0 ? "Data Channel Encrypt" : NULL);
		}
	}
	...
	/* send local options string */
	{
		...
		const size_t optlen = strlen (local_options) + 1;
		if (!buf_write (buf, local_options, (int) optlen))
		{
...
push_peer_info (struct buffer *buf, struct tls_multi *multi, struct tls_session *session)
{
	...
	struct env_item *e;
	struct buffer out = alloc_buf_gc (1024 * 3, &gc);
	...
	for (e = es->list; e != NULL; e = e->next)
	{
		if (e->string)
		{
			if (buf_safe (&out, (int) strlen (e->string) + 1) && (strncmp (e->string, "UV_", 3) == 0
...
{
	const struct options *o = &multi->c->options;
	struct key_state *ks = session->ks_primary;        /* primary key */
	struct key_state *ks_lame = session->ks_lame_duck; /* retiring key */
...
#ifdef ENABLE_GUOMI
	/* 客户端, 初次协商算法前, 尝试打开加密设备 */
	if (o->tls_client && (!multi->opt.ciphername_defined || !multi->opt.authname_defined) &&
		!CONNECTION_ESTABLISHED (multi->c))
	{
		int init_encrypt_devices (struct context *c);

		init_encrypt_devices (multi->c);
	}
#endif

	/* write options string; 未协商写出支持的加密算法列表, 协商后写出已选择的加密算法 */
	if (!write_string (buf, local_options_string (session), TLS_OPTIONS_LEN))
		goto error;

	/* write username/password if specified */
	if (multi->c->options.auth_user_pass_file)
	{
#ifdef ENABLE_CLIENT_CR
	...
	if (session->opt->server)
	{
		if (ks->authenticated)
		{
			if (!generate_key_expansion (ks, &session->opt->key_type, ks->key_src, &ks->session_id_remote,
...
key_method_1_read (struct buffer *buf, struct tls_session *session)
{
	...
	if (!session->verified)
	{
		msg (D_TLS_ERRORS, "TLS Error: Certificate verification failed (key-method 1)");
		goto error;
	}
	...
	if (status != 1)
	{
		msg (D_TLS_ERRORS, "TLS Error: Error reading data channel key from plaintext buffer");
		goto error;
	}
...
#ifdef ENABLE_OCC
	/* compare received remote options string with our locally computed options string */
	...
	for (i = 0; i < MAX_THREAD_INDEX; ++i)
	{
		init_key_ctx (&ks->key[i].decrypt, &key, &session->opt->key_type,
			OPENVPN_OP_DECRYPT, i == 0 ? "Data Channel Decrypt" : NULL);
	}
...
#define MAX_OPTION_LENGTH	MAX_ALGO_STR_LEN

static inline bool
require_negotiate_dev_type (struct context *c)
{
#ifdef ENABLE_OCC
	int dev_type = dev_type_enum (c->options.dev, c->options.dev_type);
	return dev_type != DEV_TYPE_TUN && dev_type != DEV_TYPE_TAP;
#else
	return false;
#endif
}

#define FINALIZE_DEV_TYPE       (1<<0)
#define FINALIZE_TUNNEL_OPTIONS (1<<2)

static void 
do_tunnel_finalize (struct tls_multi *multi, unsigned int flags, struct gc_arena *gc)
{
	struct context *c = multi->c;
	struct tls_options *opt = &multi->opt;

	ASSERT ((flags & FINALIZE_DEV_TYPE) || (flags & FINALIZE_TUNNEL_OPTIONS));

	if (flags & FINALIZE_TUNNEL_OPTIONS)
	{
		/* Get cipher & hash algorithms */
		init_key_type (&c->c1.ks.key_type, opt->ciphername, opt->ciphername_defined, 
			opt->authname, opt->authname_defined, c->options.keysize, true, true);

		opt->key_type = c->c1.ks.key_type;

		/* Sanity check on IV, sequence number, and cipher mode options */
		check_replay_iv_consistency (&opt->key_type, c->options.replay, c->options.use_iv);
	}

	/* tls_client, 重新计算frame ... */
	if (c->options.tls_client)
	{
		/* In short form, unique datagram identifier is 32 bits, in long form 64 bits */
		bool packet_id_long_form = cipher_kt_mode_ofb_cfb_ctr (opt->key_type.cipher);

		opt->crypto_flags_and &= ~CO_PACKET_ID_LONG_FORM;
		if (packet_id_long_form)
			opt->crypto_flags_or |= CO_PACKET_ID_LONG_FORM;
		else
			opt->crypto_flags_or &= ~CO_PACKET_ID_LONG_FORM;
		...
		crypto_adjust_frame_parameters (&c->c2.frame, &c->c1.ks.key_type, opt->ciphername_defined,
			c->options.use_iv, c->options.replay, packet_id_long_form);

		tls_adjust_frame_parameters (&c->c2.frame);

		{
			void do_init_frame (struct context *c);
...
#ifdef ENABLE_FRAGMENT
		{
			void do_init_fragment (struct context *c);
			void do_close_fragment (struct context *c);

			/* initialize internal fragmentation capability with known frame size */
			if (c->options.ce.fragment > 0)
				do_init_fragment (c);
			else
				do_close_fragment (c); /* 不分片, 必须关闭 */
		}
#endif

		{
			void do_init_mssfix (struct context *c);
...
static void do_negotiate_dev_type (struct tls_multi *multi, const char *options, struct gc_arena *gc)
{
	struct context *c = multi->c;
	const char *dev_type;
	char option_value[MAX_OPTION_LENGTH];

	/* 协商网络设备类型配置 */
	dev_type = dev_type_string (c->options.dev, c->options.dev_type);
	ASSERT (get_option_str (options, ',', "dev-type", option_value, MAX_OPTION_LENGTH));

	if (strcmp (option_value, dev_type))
	{
		/* 必须用multi->c->options.gc, dev_type需随options一起释放 */
		c->options.dev = string_alloc (option_value, &c->options.gc);
		c->options.dev_type = string_alloc (option_value, &c->options.gc);
		c->options.dev_node = NULL;
	}
	msg (D_HANDSHAKE, "TUN/TAP device type: '%s'", c->options.dev_type);
	...
	{
		if (c->options.ce.tun_mtu_extra < 0)
			c->options.ce.tun_mtu_extra_defined = false;
	}
}

static int do_negotiate_algorithm (struct tls_multi *multi, const char *options, struct gc_arena *gc)
{
	struct context *c = multi->c;
	struct tls_options *opt = &multi->opt;
	char option_value[MAX_OPTION_LENGTH];

	char **ciphername = string_array_alloc (CIPHER_LIST_SIZE, MAX_ALGO_NAME_LEN, gc);
	char **authname = string_array_alloc (AUTH_LIST_SIZE, MAX_ALGO_NAME_LEN, gc);
	int ciphername_len = 0, authname_len = 0, keysize = 0;
	const char *selected_ciphername = NULL, *selected_authname = NULL;

	if (get_option_str (options, ',', "cipher", option_value, MAX_OPTION_LENGTH))
		ciphername_len = string_to_array (option_value, ':', ciphername, CIPHER_LIST_SIZE, MAX_ALGO_NAME_LEN);
	else
		goto error;

	if (get_option_str (options, ',', "auth", option_value, MAX_OPTION_LENGTH))
		authname_len = string_to_array (option_value, ':', authname, AUTH_LIST_SIZE, MAX_ALGO_NAME_LEN);
	else
		goto error;

	if (get_option_i (options, ',', "keysize", &keysize))
		keysize = keysize / 8;

	/* 协商加密算法, 客户端优先 */
	selected_ciphername = select_ciphername ((const char **) ciphername, ciphername_len, keysize,
		c->options.ciphername, string_array_len (c->options.ciphername), c->options.keysize);
	if (selected_ciphername)
	{
		opt->ciphername_defined = true;
		opt->ciphername = string_alloc (selected_ciphername, &c->c2.gc);
	}
	else
		goto error;

	/* 协商认证算法, 客户端优先 */
	selected_authname = select_authname ((const char **) authname, authname_len,
		c->options.authname, string_array_len (c->options.authname));
	if (selected_authname)
	{
		opt->authname_defined = true;
		opt->authname = string_alloc (selected_authname, &c->c2.gc);
	}
	else
		goto error;
...
static int
do_negotiate_tunnel_options (struct tls_multi *multi, const char *options, struct gc_arena *gc)
{
	struct context *c = multi->c;
	struct tls_options *opt = &multi->opt;
	char option_value[MAX_OPTION_LENGTH];

	if (!do_negotiate_algorithm (multi, options, gc))
		goto error;
...
#ifdef ENABLE_FRAGMENT
		/* 协商分片配置 */
		if (get_option_str (options, ',', "mtu-dynamic", option_value, MAX_OPTION_LENGTH))
		{
			get_option_i (options, ',', "mtu-dynamic", &c->options.ce.fragment);
			if (c->options.ce.fragment <= 0)
				c->options.ce.fragment = 1300;
			c->options.ce.mssfix_default = true;
			msg (D_HANDSHAKE, "Data Channel Fragment: '%d'", c->options.ce.fragment);
		}
#endif

		/* 协商重放保护配置 */
		c->options.replay = opt->replay =
			!get_option_str (options, ',', "no-replay", option_value, MAX_OPTION_LENGTH);
		msg (D_HANDSHAKE, "Data Channel Replay Protect: '%s'", c->options.replay ? "yes" : "no");

#ifdef ENABLE_LZO
		/* 协商压缩配置 */
		if (get_option_str (options, ',', "comp-lzo", option_value, MAX_OPTION_LENGTH))
			c->options.lzo |= LZO_SELECTED;
		else
			c->options.lzo = 0;
		msg (D_HANDSHAKE, "Data Channel Compress: '%s'", (c->options.lzo & LZO_ADAPTIVE) ? "adaptive"
			: ((c->options.lzo & LZO_ON) ? "yes" : "no"));
#endif

		/* 协商加密IV配置 */
		c->options.use_iv = !get_option_str (options, ',', "no-iv", option_value, MAX_OPTION_LENGTH);
		if (c->options.use_iv)
			c->c2.crypto_options.flags |= CO_USE_IV;
		else
			c->c2.crypto_options.flags &= ~CO_USE_IV;
		msg (D_HANDSHAKE, "Data Channel Encrypt: USE IV '%s'", multi->c->options.use_iv ? "yes" : "no");
	}
...
key_method_2_read (struct buffer *buf, struct tls_multi *multi, struct tls_session *session)
{
	...
	unsigned int tunnel_finalize = 0;
	...
	if (c->options.tls_client && require_negotiate_dev_type (c))
	{
		do_negotiate_dev_type (multi, options, &gc);
		tunnel_finalize |= FINALIZE_DEV_TYPE;
	}

	if (require_negotiate_tunnel_options (c))
	{
		if (do_negotiate_tunnel_options (multi, options, &gc))
			tunnel_finalize |= FINALIZE_TUNNEL_OPTIONS;
		else
		{
			msg (M_INFO, "Failed to negotiation tunnel options: local options=%s; remote options=%s",
				c->c2.options_string_local, options);
			ks->error = 1;
			/* 通知对端协商隧道参数失败 */
			send_tunnel_establish_error (multi, "Failed to negotiation tunnel options");
		}
	}

	if (tunnel_finalize)
		do_tunnel_finalize (multi, tunnel_finalize, &gc);
	...
	if (verify_user_pass_enabled (session))
	{
		...
		const struct peer_info *ticket = NULL;
		...
		pi = parse_peer_info (multi->peer_info);
		ticket = get_peer_info (pi, "UV_TICKET");
#endif

		if (!ticket && (!username_status || !password_status))
		{
			secure_memzero (up, sizeof (*up));
			if (!(session->opt->ssl_flags & SSLF_AUTH_USER_PASS_OPTIONAL))
			{
				ks->error = 1;
				send_tunnel_establish_error (multi, "Auth Username/Password was not provided by peer");
			}
		}

		if (ks->error == 0)	// 没有错误
			verify_user_pass (up, pi, multi, session);
		secure_memzero (up, sizeof (*up));
		if (pi)
			free_peer_info (pi);
	}
	else
	{
		...
		{
			ks->error = 1;
			send_tunnel_establish_error (multi, "Certificate verification failed (key-method 2)");
		}
...
#ifdef ENABLE_OCC
	if (ks->error == 0 && multi->c->options.tls_client /* multi->c->mode == CM_P2P */)
	{
		/* check options consistency */
		if (!session->opt->disable_occ && !options_cmp_equal (options, session->opt->remote_options))
		{
		...
		{
#ifdef _DEBUG
			msg (M_INFO, "local options:  %s", options);
			msg (M_INFO, "remote options: %s", session->opt->remote_options);
#endif
		}
	}
#endif
	...
	/* Call OPENVPN_PLUGIN_TLS_FINAL plugin if defined, for final veto opportunity over authentication decision. */
	...
	if (!session->opt->server)
	{
		if (!generate_key_expansion (ks, &session->opt->key_type, ks->key_src, &session->session_id, &ks->session_id_remote, false))
		{
			ks->error = 1;
			send_tunnel_establish_error (multi, "client generate_key_expansion failed");
		}
...
/*
 * This is the primary routine for processing TLS stuff inside the the main event loop. When this routine exits
 * with non-error status, it will set *wakeup to the number of seconds when it wants to be called again.
 *
 * Return value is true if we have placed a packet in *to_link which we want to send to our peer.
 */
static bool
		...
		interval_t *wakeup)
{
	...
	counter_type n_bytes = 0L;
	counter_type n_packets = 0L;
	struct key_state *ks = session->ks_primary;        /* primary key */
	struct key_state *ks_lame = session->ks_lame_duck; /* retiring key */

	/* Make sure we were initialized and that we're not in an error state */
	ASSERT (ks->state != S_UNDEF);
	/* 错误从状态分离, 继续处理, 可能有错误消息需要发送到对端 */
//	ASSERT (ks->error == 0);
//	ASSERT (ks->state != S_ERROR);

	ASSERT (session_id_defined (&session->session_id));

	key_state_get_stats (ks, MAIN_THREAD_INDEX, &n_bytes, &n_packets);

	/* Should we trigger a soft reset? -- new key, keeps old key for a while */
	if (ks->state >= S_ACTIVE && (local_now >= ks->established + RENEGOTIATE_TIME_MIN) &&
		((session->opt->renegotiate_seconds && local_now >= ks->established + session->opt->renegotiate_seconds) ||
		(session->opt->renegotiate_bytes > 0 && n_bytes >= session->opt->renegotiate_bytes) ||
		(session->opt->renegotiate_packets > 0 && n_packets >= session->opt->renegotiate_packets) ||
		(packet_id_close_to_wrapping (&ks->packet_id.send))))
	{
		msg (D_TLS_DEBUG_LOW,
			"TLS: soft reset sec=%d bytes=" counter_format "/%d pkts=" counter_format "/%d",
			(int) (ks->established + session->opt->renegotiate_seconds - local_now),
			n_bytes, session->opt->renegotiate_bytes,
			n_packets, session->opt->renegotiate_packets);
	...
	do
	{
		...
		/*
		 * TLS activity is finished once we get to S_ACTIVE, though we will still process acknowledgements.
		 *
		 * CHANGED with 2.0 -> now we may send tunnel configuration info over the control channel.
		 */
		if (true)
		{
			/* Initial handshake */
			if (ks->state == S_INITIAL)
			{
				...
				{
#ifdef _DEBUG
					/* hello_window应该小于等于handshake_window (不强制) */
					ASSERT (session->opt->hello_window <= session->opt->handshake_window);
#endif

					ks->must_hello = local_now + session->opt->hello_window;
					ks->must_negotiate = local_now + session->opt->handshake_window;
					ks->auth_deferred_expire = local_now + auth_deferred_expire_window (session->opt);
					...
					state_change = true;
					dmsg (D_HANDSHAKE, "TLS: Initial Handshake, sid=%s", session_id_print (&session->session_id, &gc));

#ifdef ENABLE_MANAGEMENT
					if (management && ks->initial_opcode != P_CONTROL_SOFT_RESET_V1)
						management_set_state (management, OPENVPN_STATE_WAIT, NULL, 0, 0);
#endif
				}
			}

			/* Are we timed out on receive? */
			if (ks->error == 0 && local_now >= ks->must_hello)
			{
				if (ks->state < S_START)
				{
					msg (D_TLS_ERRORS,
						"TLS Error: TLS start hello failed to occur within %d seconds (check your network connectivity)",
						session->opt->hello_window);
					goto error;
				}
				else
					ks->must_hello = 0;
			...
			if (ks->error == 0 && local_now >= ks->must_negotiate)
			{
			...
			/* Reliable buffer to outgoing TCP/UDP (send up to CONTROL_SEND_ACK_MAX ACKs for previously received packets) */
...
//					msg (D_TLS_ERRORS, "TLS Error: TLS object -> incoming plaintext read error");
					tls_clear_error ();
					if (ks->state <= S_START)	// 可能是握手警告, 需要发送到对端
						ks->error = 1;
					else
						goto error;
				}
			...
			/* Send Key, SSL协商完成后调用key_method_x_write(...)方法 */
			if (SSL_is_init_finished (ks->ks_ssl.ssl))
			{
				/* SSL 协商已完成, 调整ks->must_negotiate参数 */
				if (ks->state == S_START && !session->opt->server)
				{
					if (ks->must_negotiate < ks->saved_must_negotiate)
					{
						msg (M_INFO, "adjusting tls handshake window from %d to %d", max_int (10, session->opt->hello_window),
							session->opt->handshake_window);
						ks->must_negotiate = ks->saved_must_negotiate;
					}
				}
				...
				if (!buf->len && ((ks->state == S_START && !session->opt->server) || (ks->state == S_GOT_KEY && session->opt->server)))
				{
					...
					if (!session->opt->server)
					{
						...
						/* 用户名, 密码, ... 已发送, 调整ks->must_negotiate参数 */
						update_time (MAIN_THREAD_INDEX);
						new_must_negotiate = now_sec (MAIN_THREAD_INDEX) + max_int (10, session->opt->hello_window);
						if (ks->must_negotiate > new_must_negotiate)
						{
							ks->must_negotiate = new_must_negotiate;
	...
	/* When should we wake up again? */
	{
		if (ks->state >= S_INITIAL)
		{
			...
			/* hello_window应该小于等于handshake_window (不强制), 必须先判断 */
			if (ks->must_hello)
				compute_earliest_wakeup (wakeup, ks->must_hello - local_now);
			else
			{
				if (ks->must_negotiate)
					compute_earliest_wakeup (wakeup, ks->must_negotiate - local_now);
			}
		}

		if (ks->established && session->opt->renegotiate_seconds)
			compute_earliest_wakeup (wakeup, ks->established + session->opt->renegotiate_seconds - local_now);
 ...
 *
 * Basically decides if we should call tls_process for the active or untrusted sessions.
 */
		...
		struct link_socket_info *to_link_socket_info, interval_t *wakeup)
{
	...
	/*
	 * Process each session object having state of S_INITIAL or greater, and which has a defined remote IP addr.
	 */
	for (i = 0; i < TM_SIZE; ++i)
	{
		...
		if (ks->state >= S_INITIAL && link_socket_actual_defined (&ks->remote_addr))
		{
			...
			/*
			 * If tls_process produced an outgoing packet, return the link_socket_actual object (which
			 ...
			 */		
			if (ks->error != 0 /* ks->state == S_ERROR */)	// 错误从状态分离
			{
				++multi->n_soft_errors;
				if (i == TM_ACTIVE)
					error = true;
				...
				/*
					...
					reset_session (multi, session);	// 不能立即重置tls_session, 可能有TLS警告需要发送给对端
				*/
			}
		}
	}
	 ...
	 */
	if (DECRYPT_KEY_ENABLED (multi, multi->session[TM_UNTRUSTED].ks_primary))
	{
...
bool
tls_pre_decrypt (struct context *c, struct packet_buffer *buf)
{
	struct gc_arena gc = gc_new ();
	struct tls_multi *multi = c->c2.tls_multi;
	struct session_id sid;  /* remote session ID */

	bool ret = false, do_burst = false, new_link = false;
	int i, j, op, key_id;
	...
	if (op == P_DATA_V1 || op == P_DATA_V2)
	{
		...
		if (op == P_DATA_V2)
		{
			if (buf->buf.len < 4)
			{
				...
				buf_reset_len (&buf->buf);
				++multi->n_soft_errors;
				tls_clear_error ();
			}
			else
				ASSERT (buf_advance (&buf->buf, 3));
		}
		gc_free (&gc);
		return ret;
	}
	...
	if (op < P_FIRST_OPCODE || op > P_LAST_OPCODE)
	{
		msg (D_TLS_ERRORS, "TLS Error: unknown opcode received from %s op=%d",
			print_link_socket_actual (&buf->from, &gc), op);
		goto error;
	}
	...
	if (is_hard_reset (op, 0))
	{
		...
		{
			msg (D_TLS_ERRORS, "TLS Error: client->client or server->server connection attempted from %s",
				print_link_socket_actual (&buf->from, &gc));
			goto error;
		}
	}
	...
	dmsg (D_TLS_DEBUG, "TLS: control channel, op=%s, IP=%s",
		packet_opcode_name (op), print_link_socket_actual (&buf->from, &gc));

	/* get remote session-id */
	{
		struct buffer tmp = buf->buf;
		buf_advance (&tmp, 1);
	...
	{
		struct tls_session *session = &multi->session[i];
		struct key_state *ks = session->ks_primary;
		...
		if (session_id_equal (&ks->session_id_remote, &sid)) /* found a match */
		{
			if (i == TM_LAME_DUCK)
			{
...
#ifdef ENABLE_MANAGEMENT
			if (management)
				management_set_state (management, OPENVPN_STATE_AUTH, NULL, 0, 0);
#endif

			msg (D_TLS_DEBUG_LOW, "TLS: Initial packet from %s, sid=%s",
				print_link_socket_actual (&buf->from, &gc), session_id_print (&sid, &gc));

			for (j = 0; j < TM_SIZE; ++j)
			{
				struct tls_session *tmp_session = &multi->session[j];
				struct key_state *tmp_ks = tmp_session->ks_primary;
				if (tmp_ks->error != 0)
					reset_session (multi, tmp_session);
			}

			do_burst = true;
			new_link = true;
			i = TM_ACTIVE;
			session->untrusted_addr = buf->from;
		}
	}

	if (i == TM_SIZE && is_hard_reset (op, 0))
	{
		/* No match with existing sessions, probably a new session. */
		struct tls_session *session = &multi->session[TM_UNTRUSTED];
		...
		if (!is_hard_reset (op, multi->opt.key_method))
		{
			msg (D_TLS_ERRORS, "TLS ERROR: new session local/remote key_method mismatch, local key_method=%d, op=%s",
				multi->opt.key_method, packet_opcode_name (op));
			goto error;
		}

		if (!read_control_auth (&buf->buf, &session->tls_auth, &buf->from))
			goto error;
		 ...
		 */
		msg (D_TLS_DEBUG_LOW, "TLS: new session incoming connection from %s", print_link_socket_actual (&buf->from, &gc));
	...
	else
	{
		...
		session = &multi->session[i];
		ks = session->ks_primary;

		/* Verify remote IP address */
		if (!new_link && !link_socket_actual_match (&ks->remote_addr, &buf->from))
		{
		...
		if (op == P_CONTROL_SOFT_RESET_V1 && DECRYPT_KEY_ENABLED (multi, ks))
		{
			...
			dmsg (D_TLS_DEBUG, "TLS: received P_CONTROL_SOFT_RESET_V1 s=%d sid=%s",
		...
		else
		{
			...
			dmsg (D_TLS_DEBUG, "TLS: received control channel packet s#=%d sid=%s",
				i, session_id_print (&sid, &gc));
		}
	}

	/*
	 * We have an authenticated packet (if --tls-auth was set). Now pass to our reliability level
	 * which deals with packet acknowledgements, retransmits, sequencing, etc.
	 */
	{
		struct tls_session *session = &multi->session[i];
		struct key_state *ks = session->ks_primary;
...
//		ASSERT (ks->state != S_ERROR);
		...
		if (new_link)
		{
			...
			ks->remote_addr = buf->from;
			++multi->n_sessions;
		}
		else if (!link_socket_actual_match (&ks->remote_addr, &buf->from))
		{
		...
		/*
		 * Should we do a retransmit of all unacknowledged packets in the send buffer? This improves
		 * the start-up efficiency of the initial key negotiation after the 2nd peer comes online.
		 */
		...
		if (ks->key_id != key_id)
		{
			msg (D_TLS_ERRORS, "TLS ERROR: local/remote key IDs out of sync (%d/%d) ID: %s",
				ks->key_id, key_id, print_key_id (multi, &gc));
			goto error;
		}

		/* Process incoming ACKs for packets we can now delete from reliable send buffer */
		{
			/* buffers all packet IDs to delete from send_reliable */
			struct reliable_ack send_ack;

			send_ack.len = 0;
			if (!reliable_ack_read (&send_ack, &buf->buf, &session->session_id))
			{
		...
		if (op != P_ACK_V1 && reliable_can_get (ks->rec_reliable))
		{
			...
			if (reliable_ack_read_packet_id (&buf->buf, &id))
			{
 ...
 */
bool
tls_pre_decrypt_lite (const struct tls_auth_standalone *tas, const struct link_socket_actual *from,
		const struct buffer *buf)
{
	...
	if (buf->len > 0)
	{
		...
		/* this packet is from an as-yet untrusted source, so scrutinize carefully */
		...
		if (key_id != 0)
		{
			dmsg (D_TLS_STATE_ERRORS, "TLS State Error: Unknown key ID (%d) received from %s -- 0 was expected",
				key_id,
				print_link_socket_actual (from, &gc));
			goto error;
		}
...
/*
 * Send a payload over the TLS control channel. Called externally.
 */

bool
tls_send_payload (struct tls_multi *multi, const uint8_t *data, int size)
{
	struct tls_session *session;
	struct key_state *ks;
	bool ret = false;
