...
/*
 * Some OSes will prefer select() over poll() when both are available.
 */
...
static int
we_wait (struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen, bool *wakeup)
{
	struct we_set *wes = (struct we_set *) es;
	...
	status = WSAWaitForMultipleEvents ((DWORD) wes->n_events, wes->events, FALSE, (DWORD) timeout, FALSE);

	/* If at least one event is already set, we must individually poll the whole list. */
	if (status >= WSA_WAIT_EVENT_0 && status < WSA_WAIT_EVENT_0 + (DWORD) wes->n_events)
	{
		...
		for (i = 0; i < wes->n_events; ++i)
		{
			...
			if ((status - WSA_WAIT_EVENT_0) == i || WaitForSingleObject (wes->events[i], 0) == WAIT_OBJECT_0)
			{
				if (i == 0)
				{
					ResetEvent (wes->events[0]);
					if (wakeup)
						*wakeup = true;
				}
				else
	...
	{
		return (status == WSA_WAIT_TIMEOUT) ? 0 : -1;
	}
}

static void
we_wakeup (struct event_set *es)
{
	struct we_set *wes = (struct we_set *) es;

	if (!SetEvent (wes->events[0]))
	{
		struct gc_arena gc = gc_new ();
		int err = GetLastError ();
		dmsg (D_WIN32_IO, "wakeup fail! %s", strerror_win32 (err, &gc));
		gc_free (&gc);
	}
}
...
we_init (int *maxevents, unsigned int flags)
{
	...
	*maxevents = min_int (*maxevents, WSA_MAXIMUM_WAIT_EVENTS);
	...
	wes->n_events = 1;
	wes->events[0] = CreateEvent (NULL, TRUE, FALSE, NULL);
...
ep_reset (struct event_set *es)
{
	struct ep_set *eps = (struct ep_set *) es;
	ASSERT (eps->fast);
	ep_ctl ((struct event_set *) eps, eps->evfd, EVENT_READ, &eps->evfd);
}

static int
ep_wait (struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen, bool *wakeup)
{
	struct ep_set *eps = (struct ep_set *) es;
	int stat, len, i, j = 0;
	...
	if (stat > 0)
	{
		...
		{
			out->rwflags = 0;
			if (ev->data.ptr == &eps->evfd)
			{
				if (ev->events & (EPOLLIN|EPOLLPRI|EPOLLERR|EPOLLHUP))
				{
					len = read (eps->evfd, &eps->count, sizeof (eps->count));
					if (wakeup)
						*wakeup = true;
				}
			}
			else
			{
				...
				dmsg (D_EVENT_WAIT, "EP_WAIT[%d] rwflags=0x%04x ev=0x%08x arg=" ptr_format,
					i, out->rwflags, ev->events, (ptr_type)ev->data.ptr);
				++out;
				++j;
			}
			++ev;
...
static void
ep_wakeup (struct event_set *es)
{
	static const uint64_t count = 0x31;
	struct ep_set *eps = (struct ep_set *) es;

	ASSERT (write (eps->evfd, &count, sizeof (count)) >= 0 || errno == EAGAIN);
}
...
ep_init (int *maxevents, unsigned int flags)
{
	...
	eps->evfd = eventfd (0x31, EFD_NONBLOCK);
	if (eps->evfd < 0)
		msg (M_ERR, "EVENT: eventfd failed, errno=%d", errno);
	ep_ctl ((struct event_set *) eps, eps->evfd, EVENT_READ, &eps->evfd);
...
static int
po_wait (struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen, bool *wakeup)
{
	struct po_set *pos = (struct po_set *) es;
	int stat, len, i, j = 0;
		...
		for (i = 0; i < pos->n_events && j < outlen; ++i)
		{
			if (pfdp->revents & (POLLIN|POLLPRI|POLLERR|POLLHUP|POLLOUT))
			{
				...
				if (i == 0)
				{
					...
					{
						len = read (pos->evfd, &pos->count, sizeof (pos->count));
						if (wakeup)
							*wakeup = true;
					}
				}
...
static void 
po_wakeup (struct event_set *es)
{
	static const uint64_t count = 0x31;
	struct po_set *pos = (struct po_set *) es;

	ASSERT (write (pos->evfd, &count, sizeof (count)) >= 0 || errno == EAGAIN);
}
...
po_init (int *maxevents, unsigned int flags)
{
	...
	pos->evfd = eventfd (0x31, EFD_NONBLOCK);
	if (pos->evfd < 0)
		msg (M_ERR, "EVENT: eventfd failed, errno=%d", errno);

	pfdp = &pos->events[pos->n_events];
	pfdp->fd = pos->evfd;
	po_set_pollfd_events (pfdp, EVENT_READ);
	pos->args[pos->n_events] = NULL;
	++pos->n_events;
...
struct se_set
{
	...
	/* 实现wakeup功能, pipefd[0]用于读取管道, pipefd[1]用于写入管道 */
	int pipefd[2];
	char x_buf[MAX_PIPE_BUF_LEN];
};
...
se_reset (struct event_set *es)
{
	...
	se_ctl ((struct event_set *) ses, ses->pipefd[0], EVENT_READ, NULL);
}

static int
se_wait_return (struct se_set *ses, fd_set *rd_set, fd_set *wr_set, struct event_set_return *out, int outlen, bool *wakeup)
{
	int len, i, j = 0;
	for (i = 0; i <= ses->maxfd && j < outlen; ++i)
	{
		const bool r = FD_ISSET (i, rd_set);
		const bool w = FD_ISSET (i, wr_set);
		if (r || w)
		{
			out->rwflags = 0;
			if (i == ses->pipefd[0])
			{
				if (r)
				{
					len = read (ses->pipefd[0], ses->x_buf, MAX_PIPE_BUF_LEN);
					if (wakeup)
						*wakeup = true;
				}
			}
...
static int
se_wait_fast (struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen, bool *wakeup)
{
	struct se_set *ses = (struct se_set *) es;
	struct timeval tv_tmp = *tv;
	int stat;

	dmsg (D_EVENT_WAIT, "SE_WAIT_FAST maxfd=%d tv=%d/%d", ses->maxfd, (int) tv_tmp.tv_sec, (int) tv_tmp.tv_usec);

	stat = select (ses->maxfd + 1, &ses->readfds, &ses->writefds, NULL, &tv_tmp);

	if (stat > 0)
		stat = se_wait_return (ses, &ses->readfds, &ses->writefds, out, outlen, wakeup);

	return stat;
}

static int
se_wait_scalable (struct event_set *es, const struct timeval *tv, struct event_set_return *out, int outlen, bool *wakeup)
{
	struct se_set *ses = (struct se_set *) es;
	struct timeval tv_tmp = *tv;
	...
	dmsg (D_EVENT_WAIT, "SE_WAIT_SCALEABLE maxfd=%d tv=%d/%d", ses->maxfd, (int) tv_tmp.tv_sec, (int) tv_tmp.tv_usec);

	stat = select (ses->maxfd + 1, &read, &write, NULL, &tv_tmp);
	if (stat > 0)
		stat = se_wait_return (ses, &read, &write, out, outlen, wakeup);

	return stat;
}

static void
se_wakeup (struct event_set *es)
{
	static const unsigned char c = 0x31;
	struct se_set *ses = (struct se_set *) es;

	ASSERT (write (ses->pipefd[1], &c, 1) >= 0 || errno == EAGAIN);
}
...
se_init (int *maxevents, unsigned int flags)
{
	...
	*maxevents = min_int (*maxevents, SELECT_MAX_FDS);
	...
	/* 创建管道 */
	ASSERT (pipe (ses->pipefd) == 0);
	set_nonblock (ses->pipefd[0]);
	set_nonblock (ses->pipefd[1]);
