...
setup_engine (const char *engine)
{
	...
	if (engine)
	{
		...
		{
			crypto_msg (M_FATAL, "OpenSSL error: ENGINE_set_default failed on engine '%s'",
				engine);
		}
...
#ifdef ENABLE_GUOMI
static ENCRYPT_DEVICE_LOCKING_CALLBACK static_device_locking_cb = {0};

static void edl_lock_lock_impl (void)
{
#ifdef PERF_STATS_CHECK
	MUTEX_LOCK (&g_encrypt_device_mutex, THREAD_SELF_INDEX (), S_ENCRYPT_DEVICE);
#else
	MUTEX_LOCK (&g_encrypt_device_mutex, -1, S_ENCRYPT_DEVICE);
#endif
}

static void edl_lock_unlock_impl (void)
{
#ifdef PERF_STATS_CHECK
	MUTEX_UNLOCK (&g_encrypt_device_mutex, THREAD_SELF_INDEX (), S_ENCRYPT_DEVICE);
#else
	MUTEX_UNLOCK (&g_encrypt_device_mutex, -1, S_ENCRYPT_DEVICE);
#endif
}

static void edl_cond_wait_impl (void)
{
	ASSERT (pthread_cond_wait (&g_encrypt_device_cond, &g_encrypt_device_mutex) == 0);
}

static void edl_cond_signal_impl (void)
{
	ASSERT (pthread_cond_signal (&g_encrypt_device_cond) == 0);
}

static void edl_cond_broadcast_impl (void)
{
	ASSERT (pthread_cond_broadcast (&g_encrypt_device_cond) == 0);
}
#endif
...
crypto_init_lib (void)
{
	...
	openssl_static_lock_steup ();
	CRYPTO_set_dynlock_create_callback (openssl_dynlock_create);
	CRYPTO_set_dynlock_lock_callback (openssl_dynlock_locking);
	...
	/*
	 * If you build the OpenSSL library and OpenVPN with CRYPTO_MDEBUG,
	 * you will get a listing of OpenSSL memory leaks on program termination.
	 */
#ifdef CRYPTO_MDEBUG
...
/*
 *
 * OpenSSL memory debugging.  If dmalloc debugging is enabled, tell OpenSSL to use our private
 * malloc/realloc/free functions so that we can dispatch them to dmalloc.
 *
 */
...
{
	const EVP_CIPHER * const *cipher_a = (const EVP_CIPHER * const *) a;
	const EVP_CIPHER * const *cipher_b = (const EVP_CIPHER * const *) b;

	const char *cipher_name_a = translate_cipher_name_to_openvpn (EVP_CIPHER_name (*cipher_a));
	const char *cipher_name_b = translate_cipher_name_to_openvpn (EVP_CIPHER_name (*cipher_b));
...
print_cipher (const EVP_CIPHER *cipher)
{
	const char *var_key_size =
		(EVP_CIPHER_flags (cipher) & EVP_CIPH_VARIABLE_LENGTH) ? " by default" : "";
	const char *ssl_only =
		cipher_kt_mode_cbc (cipher) ? "" : ", TLS client/server mode only";
...
#define CIPHER_LIST_LEN	1000
	...
	for (nid = 0; nid < 10000; ++nid)
	{
		...
		if (cipher && (cipher_kt_mode_cbc (cipher)
			...
			|| cipher_kt_mode_ofb_cfb_ctr (cipher)
#endif
			))
		{
...
/*
 *
 * Random number functions, used in cases where we want reasonably strong cryptographic 
 * random number generation without depleting our entropy pool.  Used for random
...
cipher_kt_get (const char *ciphername)
{
	...
	if (EVP_CIPHER_key_length (cipher) > MAX_CIPHER_KEY_LENGTH)
		msg (M_FATAL,
			"Cipher algorithm '%s' uses a default key size (%d bytes) which is larger than " PACKAGE_NAME "'s current maximum key size (%d bytes)",
			ciphername,
...
{
	/* OpenSSL reports OFB/CFB/CTR/GCM cipher block sizes as '1 byte'.  To work
...
void
cipher_ctx_init (EVP_CIPHER_CTX *ctx, uint8_t *key, int key_len, const EVP_CIPHER *kt, int enc)
{
...
void
cipher_des_encrypt_ecb (const unsigned char key[DES_KEY_LENGTH], unsigned char *src, unsigned char *dst)
{
...
evp_pkey_dup (EVP_PKEY *pkey)
{
	...
	if (pkey)
	{
		...
		if (EVP_PKEY_type (type) == EVP_PKEY_EC)
		{
			...
			dup = EVP_PKEY_new ();
			if (ec_key)
				EVP_PKEY_assign_EC_KEY (dup, ec_key);
		}
		else if (EVP_PKEY_type (type) == EVP_PKEY_RSA)
		{
			RSA *rsa = EVP_PKEY_get1_RSA (pkey);

			dup = EVP_PKEY_new ();
			if (rsa)
				EVP_PKEY_assign_RSA (dup, rsa);
		}
	}
