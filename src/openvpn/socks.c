...
socks_proxy_new (const char *server, int port, const char *authfile, bool retry)
{
...
static bool
socks_handshake (struct socks_proxy_info *p, socket_descriptor_t sd, volatile int *signal_received)
{
	char buf[2];
	int len = 0;
	const int timeout_sec = 5;
...
static bool
recv_socks_reply (socket_descriptor_t sd, struct openvpn_sockaddr *addr, volatile int *signal_received)
{
	char atyp = '\0';
	int alen = 0;
	int len = 0;
		...
		volatile int *signal_received)
{
...

	{
		...
		/* VER = 5, CMD = 3 (UDP ASSOCIATE), RSV = 0, ATYP = 1 (IP V4), BND.ADDR = 0, BND.PORT = 0 */
		const ssize_t size = send (ctrl_sd, "\x05\x03\x00\x01\x00\x00\x00\x00\x00\x00", 10, MSG_NOSIGNAL);
		if (size != 10)
		{
			msg (D_LINK_ERRORS | M_ERRNO, "establish_socks_proxy_passthru: TCP port write failed on send()");
			goto error;
...
error:
	buf->len = 0;
	buf_set_tracking (buf, PACKET_DROP_SOCKS_ERROR);
}
